...
页面中的新版雨城通、整合地图和工作台地址都从 `yctBaseUrl` 派生，不需要逐个修改 HTML。电报纸生成器已迁移到 `services/telegraph`，也从 `yctBaseUrl` 派生；线网数据编辑器、动态线路图和物料图鉴继续从 `legacyBaseUrl` 派生，Nginx 的旧静态文件兜底必须继续提供这些路径，不能交给 Next.js 吞掉。
...
- 路牌、楼牌和公交站牌旧生成器已经由新版工作台覆盖，门户不再重复展示；电报纸生成器已迁移为雨城通的独立工具卡片。
- 路牌与楼牌工作台支持自由填写道路、地名和编号，也支持为道路指示牌选择安装坐标、为楼牌关联服务器标记点。
- 物料与周边图鉴记录临东的视觉物料、纪念品和实体周边，归入服务器服务，而不是通用创作工具。
- 公共交通导视工作台可以把 RMP 项目作为只读自定义线网，也能把 NaL 导向标志设计器和 Chitose.City Sign Maker 的工程近似转换为可继续编辑的导视牌。门户以文本链接列出兼容来源，不使用第三方 Logo，也不暗示与这些工具存在官方合作；具体版本、可转换组件与导入限制以工作台说明为准。
...
部分外部或独立服务仅提供简体中文内容或界面。门户在繁体中文和英文界面为 Wiki、创作工具及物料与周边图鉴显示“简体中文”标识；简体中文页面隐藏该标识。社群区另外说明其主要使用简体中文交流，同时明确欢迎其他语言的访客参与。
...
四个主工具使用真实成品作为封面，文件位于 `assets/tools`，来源与处理信息见同目录的 `manifest.json`。媒体区通过 `--tool-media-surface` 提供统一的 `#eeeeee` 浅灰底色，后续主题可以覆盖这个变量而不必重新导出图片。更新图片时继续保留明确的替代文本，不使用模拟生成结果或整页浏览器边框。
...
- `portal:friend-link-copy-requested`：用户请求复制待确认或不活跃友链地址。
- `portal:friend-link-copied`：友链地址复制动作完成，并返回是否成功。
...
公开列表可以包含互换友链和临东主动推荐的相关站点。“待确认”只表示当前访问状态无法由自动检查确认，“不活跃”表示项目或站点已明确停止活跃；两者都不表示对方没有回链。带这两种状态的友链在门户中不直接跳转，而是通过剪贴板按钮复制地址，降低失活域名被接管后误导访问的风险；复制失败时会把完整地址显示出来供人工检查。关系类型、对方回链状态和最近核验日期应在维护记录中管理，不在公开页面标注；页面通过社交账号入口接受新的友链交换联系。
