...
| `MaterialDraftCreated`                       | 用户首次同步本机物料草稿      | 记录草稿创建、触发后续同步或审计监听；Payload 携带 `draftId`、可选 `clientDraftId`、模板版本、创建者和创建时间                                                                                                                                    |
| `MaterialDraftUpdated`                       | 用户重试同步或修改已同步的草稿 | 更新草稿读取模型和审计；Payload 只携带稳定标识、模板版本、变更字段和更新时间                                                                                                                                                               |
