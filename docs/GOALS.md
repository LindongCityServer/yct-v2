...
- 地图瓦片生产环境无法直接提供 HTTPS；HTTP 瓦片图、玩家标记等动态地图数据已确认通过 YCT 后端代理处理，同时可优先使用现有静态 HTTPS 源作为安全默认源。当前工程已先为较新 HTTP 瓦片补 `/api/map/tile-proxy` 同源代理。
...
- 管理员 PIN 来源。
- 票务库存、核销链接、退票和对账细则。
- 已确认补充：HTTP 瓦片图、玩家标记等动态地图数据通过 YCT 后端代理处理；瓦片图也可以优先使用现有静态 HTTPS 源作为安全默认源。较新 HTTP 瓦片的首版代理已补到 `/api/map/tile-proxy`。
