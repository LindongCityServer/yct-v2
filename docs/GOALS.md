...
- 已处理第一版：公交、地铁、有轨、客运、地方铁路等线路介绍已有地图探索二级页面 `/map/lines/[id]`；仍待处理出行页结构精简，后续需要把出行页收窄到行程提醒、客运班次查询、订票功能，并为轮渡、航班等后续交通方式预留入口。
...
- 地图页面需要支持触控设备上的双指捏合缩放，并继续保持瓦片层、标记点层、道路/线路轨迹层同步缩放。
- 区域 POI 需要支持关联点 POI：可把若干点 POI 归入某个区域的出入口、楼栋、景点、设施等分组；区域 POI 可指定代表 POI，默认代表可以是同名点标记。标记碰撞检测需要优先保障代表 POI 展示。
...
- 地图道路标签需要尽量放在当前可见道路轨迹片段的中心，并保证锚点落在轨迹上；长道路不能只用整条轨迹的固定中点，否则视口内可能看不到标签。
- 地图“路网”模式不应隐藏住宅、办公楼、工厂标记点的文字标签；这三类低交通相关标签只在“交通”模式下降低优先级并按拥挤情况隐藏。
- 地图搜索需要新增“搜索特定地点周边的标记点”模式；POI 介绍页的“周边”按钮应进入该模式，并携带当前 POI 作为中心。
- 地图区域需要增加蓝色白边圆点定位标记。展示路线规划卡片时，需要隐藏 `map-marker-list`，避免路线面板和标记列表争抢空间。
- `/search` 搜索结果页需要增加分类 chip 栏，避免某一类结果过多时压住其他类型结果。
- `account-badge.is-dot` 在浅色模式下背景色与标题栏容易混淆，需要调整对比度或描边。
- `/travel/schedules` 的 `schedule-trip-card` 样式后续参考用户 2026-07-03 附图：左侧显示交通方式图标、班次号/线路名；中间强调出发时间、运行时间和经停；右侧强调到达时间和票价；底部保留添加提醒、保存记录、前往订票等操作。
- `/account` 通知与免打扰区域的文案需要更明确：区分“允许哪些类型通知”和“免打扰时段是否阻止推送/延后提醒”，避免用户以为开关只控制免打扰。
