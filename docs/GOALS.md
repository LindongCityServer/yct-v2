...
- 已处理补充：道路图与标记点道路投影索引前移到地图数据阶段构建；每个点状标记会按道路归并出固定投影候选，每条道路只保留一个最近投影，除最近投影外仅保留 50 格范围内的其它道路投影。路线端点、出入口接驳、公交/客运站点间道路段会优先使用这些固定投影，减少因临时按目标方向重选道路导致的斜穿街区。父地点与自己的子地点互相规划时不强制经道路投影；出入口类子地点也不会自动改走同父地点的其它出入口。交通方式开关改为短延迟批量重算，连续切换多个方式时会合并为一次规划；路线方案摘要时间改为按已生成步行/道路接驳结果估算，降低摘要时间比详情明显偏短的问题。
