...
- 管理员 PIN 来源。
- 票务库存、核销链接、退票和对账细则。
- 已确认补充：HTTP 源只负责实时瓦片图和玩家位置，地点标记与精细静态瓦片图继续来自 `map.shangxiaoguan.top`；HTTP 瓦片图、玩家标记等动态地图数据通过 YCT 后端代理处理，瓦片图也可以优先使用现有静态 HTTPS 源作为安全默认源。较新 HTTP 瓦片的首版代理已补到 `/api/map/tile-proxy`。
