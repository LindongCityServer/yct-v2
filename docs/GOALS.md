...
- 已处理第一版：`map-route-plan-card` 的圆角、图标和字号已收敛到地图浮层尺度，并限制最大高度；移动端 `map-control-stack` / `map-sidebar-stack` 改为贴在底部导航上方，避免路线卡片被底部导航遮挡。起点/终点行已按输入控件视觉处理，后续仍需接入“聚焦后输入地点、搜索候选、点选地图或标记回填”的完整状态机。
