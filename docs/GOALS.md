...
- 已处理补充：POI 后台审核卡片新增可能冲突提示，会读取 `/api/map/markers` 公开地图标记快照，按同名、120 格内近距离、同分类 220 格内附近三类条件提示最多 5 个已有标记，并提供跳转地图聚焦链接；管理员现在可以对单条冲突提示标记“已忽略”“待合并”或“重置”，决策写入 `.yct-data/poi-conflict-decision-store.json` 并发布 `PoiConflictDecisionUpdated` 事件。真正合并、替换和父地点推导仍待后续后台状态机补齐。
