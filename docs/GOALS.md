...
- 待处理：蓝点、起点、终点以及未来可能出现的轨迹点/途经点等临时标记图层，需要和瓦片层、普通标记层、道路/线路轨迹层共用统一的移动、拖拽和缩放动画，避免交互过程中不同图层视觉脱节。
- 待处理：`map-poi-panel` 和 `map-route-plan-card` 需要增加面板级临时收起/展开按钮，方便用户在查看地点详情或路线草稿时快速腾出地图视野。
