...
- 已处理第一版：地图区域用蓝色白边圆点标示“回到默认视图”按钮对应的默认落点；路线规划草稿在地图上额外显示 `origin` 起点图标与 `flag` 终点图标，路线草稿卡片的起终点行同步使用这套端点图标，并在草稿卡片显示时隐藏 `map-marker-list`，避免路线面板和标记列表争抢空间。
...
- 已处理补充：分享图生成使用 `html-to-image`，并在导出前通过 `getFontEmbedCSS()` 嵌入本地 `Material Symbols Outlined` 字体，避免生成图中把 `subway`、`flag` 等 ligature 当成普通文本渲染。分享图内的标题图标、起点、终点、上车/下车和步行详情继续使用本地图标字体；起点统一使用 `origin`，终点统一使用 `flag`。步行详情文案继续只保留左转、右转、左前方、右前方、直行等方向语义。主路线卡片仍保持原有“图标 + 简短文案”结构。
