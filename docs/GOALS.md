...
- 已处理补充：POI 后台图片预览新增本地上传图片基础审计信息，`/api/admin/map/poi-submissions` 会为本地上传图片附带存储文件名、MIME 类型、文件大小和文件更新时间；后台卡片会展示这些信息，外部图片链接会提示无法读取本地元数据。多图、图片审核状态、图片说明、删除和替换仍待后续图片素材审核流。
