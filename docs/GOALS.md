...
- 已处理第一版：子 POI 详情页会显示所属父 POI，并可点按返回父地点；全局搜索页的线路结果改为打开地图并聚焦对应线路 marker；道路和线路详情复用 POI 操作栏，支持查看路线、搜索周边、收藏和分享。
...
- 已处理规划第一版：新增 `docs/I18N_PLAN.md`，明确多语言第一阶段支持 `zh-CN`、`zh-Hant` 和 `en`，固定界面文案走翻译目录，地名、组织名、站名等业务数据必须走正式译名字段、别名表、后台维护或审核投稿，不能先用机器翻译直接替换。语言切换控件中的语言名称固定使用自称名称：`简体中文`、`繁體中文`、`English`。契约层已补 `LocalePreferenceUpdated`、`TranslationCatalogPublished` 和 `EntityTranslationUpdated` 事件，后续用于账号偏好同步、翻译目录发布、实体译名审核和搜索索引刷新。客户端已新增 `common` 翻译目录与 `useI18n` hook，并先接入主导航、顶部搜索/乘车码/账号入口、普通用户二级页返回按钮与固定标题、账号页外观/语言/字体/动态偏好控件、搜索页固定文案、离线页固定文案、首页运营信息固定文案、更多服务页固定文案和出行一级页固定文案。
- 已处理第一版：新增语言偏好基础能力，`GET/POST /api/account/locale-preference` 读取或更新登录用户的 `system`、`zh-CN`、`zh-Hant`、`en` 偏好；数据暂存到 `.yct-data/locale-preference-store.json`，更新成功发布 `LocalePreferenceUpdated` 事件。前端新增 `client-locale-preference` 工具和账号页语言偏好控件，用于匿名本地偏好和登录同步，并更新页面 `lang`。当前只落偏好和解析后的 `resolvedLocale`，尚不切换全站 UI 文案。
