...
- 当前投稿接口要求登录 `ldpass` 且已验证服务器账号；匿名用户、非验证用户和私有 POI 暂不开放提交。图片第一版支持用户文件上传到 `.yct-data/poi-submission-images`，上传成功后发布 `PoiSubmissionImageUploaded` 事件并把生成的图片 URL 写入 POI 投稿；图片安全检查、对象存储、营业时间、电话、出入口/设施关联、代表 POI、线性/区域几何的可视化编辑仍待后续后台与表单设计。新增待办：公开 POI 投稿表单需要提供地图选点按钮回填坐标；后台中涉及坐标、站点和线路途径点的字段也应复用地图选点控件，并支持按特定分类筛选可选标记。
...
- 已处理补充：新增 `scripts/init-admin-membership.ps1`，部署包会携带为 `init-yct-admin.ps1`，可在没有 pnpm/tsx 的 standalone 服务器上直接把指定 `ldpassUserId` 写入 `.yct-data/admin-memberships.json` 或 `YCT_ADMIN_STORE_PATH` 指向的管理员成员文件；已用临时仓储验证脚本可创建 `super_admin active` 成员。
