...
- 已处理：交通方式 Profile 第一版可通过后台维护名称、默认色、Material Symbols 图标、排序和启用状态；地方铁路默认棕色，客运默认黄绿色。当前使用 `.yct-data/transit-mode-profile-store.json` 作为数据库接入前的本地仓储，成功更新后发布 `TransitModeProfileUpdated` 事件。
...
- 新增方向：客运、轮渡、航班等班次查询和票务能力需要通过同一套新版查询订票平台解决，不沿用旧 `/ltcx/` 的纯前端本地订单逻辑。已新增 `docs/TRAVEL_TICKETING_PLATFORM.md` 记录统一模型、状态机、事件、踩坑点和测试用例，并已在 `packages/contracts/src/events.ts` 中补充第一版票务事件契约。
