...
- 当前前台入口位于 `/map` 的地图浏览/图层面板内，点击“投稿 POI”打开弹窗；表单只采集地点名称、允许公开投稿的 POI 分类、Minecraft X/Z 坐标，并以 `public_pending_review` 的点状公开 POI 提交审核。
- 当前投稿接口要求登录 `ldpass` 且已验证服务器账号；匿名用户、非验证用户和私有 POI 暂不开放提交。描述、图片、营业时间、电话、链接、出入口/设施关联、代表 POI、线性/区域几何的可视化编辑仍待后续后台与表单设计。
...
- 已处理第一版：`/travel/schedules` 的 `schedule-trip-card` 参考用户 2026-07-03 附图重排，顶部显示交通方式图标、班次/线路和票价，主体强调出发时间、运行/经停和到达时间，底部保留添加提醒、保存记录、旧版参考和票务状态操作。
...
- 已处理第一版：针对云服务器部署时 `@swc/helpers/__/_interop_require_default` 缺失的问题，`@swc/helpers` 已显式加入 web 依赖，并由部署打包脚本补拷贝到 standalone 的 `apps/web/node_modules`；生成的启动脚本新增 `-NodePath` 参数，用于在宝塔内置 Node 最高仅 18.6.0 时改用便携版 Node 22。
