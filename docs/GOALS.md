...
- 已处理补充：道路图与标记点道路投影索引前移到地图数据阶段构建；地图数据可见但道路接入点仍在计算时，会在地图区域显示状态提示，路线规划会等待索引 ready 后再计算候选，避免先显示过度直线化的临时结果。每个点状标记会按道路归并出固定投影候选，每条道路只保留一个最近投影，且所有投影点都必须在 50 格范围内，最近的第一个投影点不再绕过范围限制；没有范围内投影的标记点不会强行吸附远处道路。路线端点、出入口接驳、公交/客运站点间道路段会优先使用这些固定投影，减少因临时按目标方向重选道路导致的斜穿街区。父地点与自己的子地点互相规划时不强制经道路投影；出入口类子地点也不会自动改走同父地点的其它出入口。交通方式开关改为短延迟批量重算，连续切换多个方式时会合并为一次规划；路线方案摘要时间改为按已生成步行/道路接驳结果估算，降低摘要时间比详情明显偏短的问题。
