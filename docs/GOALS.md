...
- 已处理补充：POI 后台支持管理员在待审核状态下修正标题、分类、简介和链接；接口为 `PATCH /api/admin/map/poi-submissions/[poiId]`，会校验分类存在、限制仅 `pending_review` 可改，并发布 `PoiSubmissionUpdated` 事件。几何、图片、父子关系、代表 POI 和线性/区域结构仍留给后续地图编辑器。
