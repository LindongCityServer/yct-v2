...
- 当前投稿接口要求登录 `ldpass` 且已验证服务器账号；匿名用户、非验证用户和私有 POI 暂不开放提交。图片第一版支持用户文件上传到 `.yct-data/poi-submission-images`，上传时会校验常见图片文件签名，上传成功后发布 `PoiSubmissionImageUploaded` 事件并把不带 `/v2` 等部署前缀的稳定内部图片路径写入 POI 投稿，展示层再按当前 `appPath` 解析；前台公开 POI 投稿表单已新增地图选点按钮，可直接从地图空白处回填点坐标；后台 POI 审核列表已展示投稿图片缩略图并可打开原图，便于管理员人工核验。当前后台审核列表已新增代表坐标、地图查看、复制坐标和复制几何 JSON 的辅助入口，地图页支持 `x/z/label` 或 `coordinate` URL 参数显示临时坐标定位。完整图片安全扫描、对象存储、营业时间、电话、出入口/设施关联、代表 POI、线性/区域几何的可视化编辑仍待后续后台与表单设计；后台中涉及坐标、站点和线路途径点的字段也应继续复用地图选点控件，并支持按特定分类筛选可选标记。
...
- 已处理第一版：地图选点控件、POI 审核/几何编辑器、线路站点编辑和物料工作台选点统一采用“静态瓦片先显示、较新瓦片逐块异步覆盖”的策略；较新瓦片通过同源代理读取，代理失败、透明占位或非图片响应时保留静态底图，静态源不可用时才单独使用较新源。
...
- 已处理：线路详情统一通过 `/map?marker=transit-line-...` 在地图上展示线路轨迹、小面板、方向切换和站点序列；地图线路型 POI、全局搜索线路结果和出行页线路列表使用同一入口。旧 `/map/lines/[id]` 与 `/travel/[id]` 仅保留兼容既有链接。
...
- 已处理：公交、地铁、有轨、客运、地方铁路等线路介绍通过 `/map?marker=transit-line-...` 留在地图页展示；`/travel` 已收窄为“出行服务”和“行程提醒”两个模块，出行服务只保留班次查询、线路与站点地图导向、智运大屏/客运展示、票券与订单预留入口，完整线路浏览继续交给地图探索。真实订票、票券、核销、退票以及轮渡/航班更深接入仍待后续统一票务平台推进。
...
- 已处理第一版：深色模式下地图区域背景、无瓦片底色、静态瓦片滤镜、道路/公交轨迹阴影已接入地图专属变量，并兼容手动深色和系统深色；后续仍可继续评估独立暗色瓦片源、不同浏览模式的独立滤镜和真实底图抽样调色。
...
- 已处理第一版：地图路线规划卡片新增交通方式开关和多方案初步结果；当前根据直线距离、旧道路端点、已导入站点、真实线路站序和站点 `travelTime` 生成步行直达、少转弯步行、公共交通直达和一次换乘候选，按耗时/距离排序后根据启用交通方式数量动态展示多条可能路径，并在地图上用虚线折线高亮选中方案。步行与公交/客运接驳已优先把起终点投影到更贴近目标方向的最近道路段，再在旧道路端点图中按 100 格连通候选拼接过渡路径；道路间连通候选改为按线段到线段最近点生成，不再只看道路标记点的点到点距离；道路接入候选会同时保留综合方向评分较优和距离地点最近的投影点，减少从路口或拐点过早切直线的情况；无法连通的片段才回退直线估算，并明确标注不代表正式道路级导航。少转弯步行候选会在道路图节点搜索中加入转向惩罚，并且只在确实减少转弯、距离没有明显失控时追加展示。路线卡片已增加“最快到达”“最少换乘”“最少步行”等徽标；步骤时间线已从纯文本升级为地点、步行、乘车和换乘过程的结构化节点，并按参考稿调整为地点节点与过程竖条的视觉层级，乘车步骤使用对应线路色或交通方式语义色。`is-walk`、`is-transfer` 与 `is-transit` 步骤已增加可展开详情：步行和换乘步行可展示按道路分组的距离、估算耗时和粗略转向图标，公共交通可展示经过站点。地图路线高亮已拆分步行、换乘步行和每段公共交通线路：步行段改用页面强调色以提升在地图底图上的可读性，每条线路段使用自身线路色或交通方式语义色；路线结果态会对真实道路行进段提取道路文字标签，标签锚点取当前视口内该步行道路段可见部分的中点，接入道路和离开道路的直线段不贴道路名，短到不足以容纳标签的可见段会隐藏标签。路线结果态保留的起点、终点和途经站点从未碰撞过滤的投影点中选取，不再被地图碰撞隐藏规则误删；上车站和下车站强制显示文字标签，普通途经站只强制显示图标并隐藏文字标签。公共交通候选会过滤起终点同站进出的结果，线路区间切片使用过滤后的可定位站点索引，避免把实际乘坐区间外的终点站或无关站点纳入地图高亮路径；线路展示和站点匹配会优先约束到同类别站点，尽量避免地铁线误连公交站、铁路站等异类站点。路线候选计算已从 render 过程中拆出，首次规划前会缓存旧道路图并显示“正在规划”的提示，避免路线卡片在大路网下无反馈卡顿；同一轮规划中的道路接驳候选、节点路径和坐标对道路结果也会复用缓存，减少公交/换乘候选反复计算同一段路。区域/父子 POI 已按旧命名规则加入临时接驳：优先使用入口、出口、出入口、站口、门等子地点；如果没有这类子地点，则用同方向最近子地点兜底。地铁进出站第一版会优先经过地铁站出入口；地铁线路之间换乘保持站内接驳，不使用出入口绕行。路线规划卡片已支持分享当前展示路线，链接会携带起终点、交通方式和当前候选方案参数，打开后可恢复草稿并重新计算；地点和路线分享已改为选择式面板，支持复制链接、复制带语义符号的文字版和生成分享图。地点分享继续使用 `ms` 紧凑编码；路线分享会在 `rsc` 压缩编码和旧 `rs` Base64URL 参数之间选择更短的参数，并兼容旧链接；分享图复用 `map-route-step-timeline` 结构，步行段默认展开道路详情，分享反馈使用短暂全页面 toast。真正短链接后续应通过服务端保存分享状态并返回短 slug（如 `/map?s=...`）实现，而不是继续堆叠前端编码。道路图构建已补充道路尽头到另一条道路线段投影点不足 100 格时的临时连通规则；实际相交仍按线段交点连通，非相交的近距离连通只从道路尽头投影到另一条道路线段，避免把普通拐点控制点误判为可跨越路口。所有方案均明确标注为估算方案，正式道路图、后台可编辑出入口/方向规则、线路真实轨迹坐标、高阶换乘和多策略权重仍待道路网络、线路方向坐标和时刻表数据继续完善。
- 已处理补充：道路图与标记点道路投影索引前移到地图数据阶段构建；地图数据可见但道路接入点仍在计算时，会在地图区域显示状态提示，路线规划会等待索引 ready 后再计算候选，避免先显示过度直线化的临时结果。每个点状标记会按道路归并出固定投影候选，每条道路只保留一个最近投影，其中标记点最近投影默认保留，其它投影仍需在 50 格范围内；同一个投影坐标如果被多个标记点候选命中，只归属于距离该投影点最近的标记点，避免道路尽头端点被多个地点同时占用。路线端点、出入口接驳、公交/客运站点间道路段会优先使用这些固定投影，减少因临时按目标方向重选道路导致的斜穿街区。父地点与自己的子地点互相规划时不强制经道路投影；出入口类子地点也不会自动改走同父地点的其它出入口。交通方式开关改为短延迟批量重算，连续切换多个方式时会合并为一次规划；路线方案摘要时间改为按已生成步行/道路接驳结果估算，降低摘要时间比详情明显偏短的问题。
...
- 已处理第一版：地图区域用蓝色白边圆点标示“回到默认视图”按钮对应的默认落点；路线规划草稿在地图上额外显示放大的 `location_on` 起点图标与 `flag` 终点图标，端点图标以底部中点锚定对应坐标；路线草稿卡片的起点行使用 `location_on`、终点行使用 `flag`，并在草稿卡片显示时隐藏 `map-marker-list`，避免路线面板和标记列表争抢空间。
...
- 已处理规划第一版：新增 `docs/I18N_PLAN.md`，明确多语言第一阶段支持 `zh-CN`、`zh-Hant` 和 `en`，固定界面文案走翻译目录，地名、组织名、站名等业务数据必须走正式译名字段、别名表、后台维护或审核投稿，不能先用机器翻译直接替换。语言切换控件中的语言名称固定使用自称名称：`简体中文`、`繁體中文`、`English`。契约层已补 `LocalePreferenceUpdated`、`TranslationCatalogPublished` 和 `EntityTranslationUpdated` 事件，后续用于账号偏好同步、翻译目录发布、实体译名审核和搜索索引刷新。客户端已新增 `common` 翻译目录与 `useI18n` hook，并先接入主导航、顶部搜索/乘车码/账号入口、普通用户二级页返回按钮与固定标题、账号页外观/语言/字体/动态偏好控件、账号身份/登录状态/账号入口操作、推送通知与免打扰设置、PWA 安装与缓存管理、自定义离线范围列表与弹窗、本地历史统计/同步/清理操作、搜索页固定文案、离线页固定文案、首页运营信息固定文案、更多服务页固定文案、出行一级页固定文案、行程提醒面板标题/摘要/弹窗表单/列表空态/状态标签/来源标签/操作按钮固定文案、统一班次查询页标题/筛选/本地记录/提醒公告/班次卡片基础操作/票务按钮固定文案、客运大屏详情页筛选/摘要/空态/班次元信息固定文案、站点详情页层级/出入口/设施/换乘周边/数据来源固定文案、线路详情页概览/方向/站点列表/单向停靠/站点摘要/数据来源固定文案、账号页票务草稿入口和票务草稿面板、票务订单详情页、订单状态/服务类型/取消原因/库存占用状态固定文案，以及地图页标题、搜索框、标记列表空态、附近搜索提示、POI 常用操作、POI 详情属性/接驳线路/关联地点分组固定文案、已知标记分类名、已知交通方式名、路线规划卡片端点编辑/交通方式/候选方案/步骤详情/估算说明固定文案、图层与投稿面板、地图数据状态提示、地图比例尺/坐标提示和公开 POI 投稿弹窗固定文案。
- 已处理补充：运营首页强提醒卡片的时间窗文案（开始于 / 有效至）已接入翻译目录，避免英文和繁体界面残留简体中文。
- 已处理补充：地图内线路详情的首末车缺失兜底和班次数文案已复用 `lineDetail` 翻译目录，避免英文界面残留“待补 / 个班次”。
...
- 已处理补充：分享图生成使用 `html-to-image`，并在导出前通过 `getFontEmbedCSS()` 嵌入本地 `Material Symbols Outlined` 字体，避免生成图中把 `subway`、`flag` 等 ligature 当成普通文本渲染。分享图内的标题图标、起点、终点、上车/下车和步行详情继续使用本地图标字体；起点统一使用 `location_on`，终点统一使用 `flag`。步行详情文案继续只保留左转、右转、左前方、右前方、直行等方向语义。主路线卡片仍保持原有“图标 + 简短文案”结构。
- 已处理补充：Material Symbols Outlined 已从 Google Fonts 外链改为项目本地嵌入的可变 `woff2` 字体，移除公开页面的 `fonts.googleapis.com` / `fonts.gstatic.com` 依赖，保留 `FILL` 可变填充态以支持选中图标样式；固定图标通过 `icon_names` 根据源码生成按需子集。后台动态图标使用受保护的单图标预览接口，确认或业务事件触发后转换为本地哈希 SVG，公开页面优先使用本地资源并回退字体子集。
...
- 已处理第一版：POI 后台审核列表新增待审核/待发布/已发布/当前结果摘要、状态筛选、分类筛选、关键词搜索、分类图标与分类名展示、投稿详情展开和带理由的驳回弹窗；状态筛选可直接切到“阻塞发布”，摘要区也会显示图片不合格、待合并这两类发布阻塞计数；`/api/map/poi-categories` 同步返回图标基础 URL，便于后台展示分类默认图标。完整分类图标增删改、一类多图标管理、地图内嵌审核、几何编辑、重复检测、区域代表 POI、水域/道路高度约束、交通线路后台协同和地点多语种维护仍按 `docs/ADMIN_BACKOFFICE_REVIEW.md` 后续阶段推进。
- 已处理补充：POI 后台审核卡片新增可能冲突提示，会读取 `/api/map/markers` 公开地图标记快照，按同名、120 格内近距离、同分类 220 格内附近三类条件提示最多 5 个已有标记，并提供跳转地图聚焦链接；管理员现在可以对单条冲突提示标记“已忽略”“待合并”或“重置”，决策写入 `.yct-data/poi-conflict-decision-store.json` 并发布 `PoiConflictDecisionUpdated` 事件。发布公开 POI 前会拦截仍标记为待合并的冲突提示，发布确认弹窗也会提示并禁用确认发布。真正合并、替换和父地点推导仍待后续后台状态机补齐。
- 已处理补充：POI 驳回弹窗新增常用原因快捷填充，覆盖坐标偏离、分类不准、命名问题、信息不足、图片问题和疑似重复；当前先作为前端固定模板，后续再抽成后台可配置驳回模板。
- 已处理补充：POI 后台支持管理员在待审核状态下修正标题、分类、简介、链接和点状 POI 的 X/Z 坐标；接口为 `PATCH /api/admin/map/poi-submissions/[poiId]`，会校验分类存在、限制仅 `pending_review` 可改，并发布 `PoiSubmissionUpdated` 事件。坐标修正弹窗提供数字输入、小地图点选预览和地图辅助查看链接，小地图点选会显示原始点、当前点与附近真实参考标记并可点击回填坐标；完整内嵌地图选点控件、图片、父子关系、代表 POI 和线性/区域结构仍留给后续地图编辑器。
- 已处理补充：POI 后台支持管理员在待审核状态下反转线性 POI 的 `LineString` 点序；修正弹窗会展示线性点数、起点和终点，并通过同一个 `PATCH /api/admin/map/poi-submissions/[poiId]` 发布 `PoiSubmissionUpdated` 事件。补点、删点、道路吸附和区域几何编辑仍留给后续地图编辑器。
- 已处理补充：POI 投稿详情新增轻量 SVG 几何预览，支持点、点组、线、矩形、矩形组、多边形和多重多边形的大致形状展示，减少线性/区域投稿只能查看 JSON 的审核负担；后续仍需要真正叠加底图和可编辑点序的地图几何编辑器。
- 已处理补充：POI 投稿详情新增只读审核地图预览第一版，使用真实 `/api/map/markers` 快照，把投稿几何与附近标记、同分类标记、道路参考点和站点参考点投到同一个 SVG 小地图中，并列出最近几个参考标记；当前用于辅助判断大致位置关系，后续仍需升级为真正内嵌地图、瓦片/道路/站点图层和地图选点控件。
- 已处理补充：POI 投稿详情新增父子地点只读推断提示第一版，会识别 `父地点-子地点` 这类旧地图命名规则，并尝试从真实地图标记快照中找到同名父地点公开标记作为审核参考；出入口、设施、楼栋、景点、代表 POI 等正式关联维护仍留给后续后台编辑器。
- 已处理补充：POI 后台图片预览新增本地上传图片基础审计信息，`/api/admin/map/poi-submissions` 会为本地上传图片附带存储文件名、MIME 类型、文件大小和文件更新时间；后台卡片会展示这些信息，外部图片链接会提示无法读取本地元数据。多图、图片说明、删除和替换仍待后续图片素材审核流。
- 已处理补充：POI 后台图片预览新增局部审核状态第一版，管理员可以将单张投稿图片标记为“图片可用”“图片不合格”或重置状态；状态写入 `.yct-data/poi-submission-image-review-store.json`，通过 `/api/admin/map/poi-submission-image-reviews` 读写，并发布 `PoiSubmissionImageReviewed` 事件。发布公开 POI 前会拦截已标记为不合格的图片，发布确认弹窗也会提示并禁用确认发布。多图、图片说明、删除、替换、对象存储和安全扫描仍待后续完整素材审核流。
