...
- 已处理第一版：`map-route-plan-card` 的圆角、图标和字号已收敛到地图浮层尺度，并限制最大高度；移动端 `map-control-stack` / `map-sidebar-stack` 改为贴在底部导航上方，避免路线卡片被底部导航遮挡。起点/终点行已按输入控件视觉处理，并支持聚焦后输入地点、查看真实标记候选、点选候选或地图标记回填端点；空白地图坐标点选、道路吸附和区域 POI 出入口吸附仍待后续实现。
