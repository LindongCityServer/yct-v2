...
- 已处理补充：POI 后台图片预览新增局部审核状态第一版，管理员可以将单张投稿图片标记为“图片可用”“图片不合格”或重置状态；状态写入 `.yct-data/poi-submission-image-review-store.json`，通过 `/api/admin/map/poi-submission-image-reviews` 读写，并发布 `PoiSubmissionImageReviewed` 事件。多图、图片说明、删除、替换、对象存储和安全扫描仍待后续完整素材审核流。
