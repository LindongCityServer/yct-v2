...
- 地图瓦片生产环境无法直接提供 HTTPS；HTTP 源仅提供实时瓦片图和玩家位置，地点标记与精细静态瓦片图仍来自 `https://map.shangxiaoguan.top`；HTTP 瓦片图、玩家标记等动态地图数据已确认通过 YCT 后端代理处理，同时可优先使用现有静态 HTTPS 源作为安全默认源。当前工程已先为较新 HTTP 瓦片补 `/api/map/tile-proxy` 同源代理。
...
- 账号设置支持深浅色主题、强调色、字体和语言；默认强调色跟随 `ldpass` 主题计划，在青绿色、红色和灰色之间按时间切换，用户可改为自定义；默认字体使用随旧系统迁移的 `HarmonyOS Sans SC`，用户可切换回系统字体。
...
- 首页 `hero-panel` 封面已改为真实图片元素，遮罩渐变从下往上；`eyebrow` 继续使用对应消息分类。
- `feed-panel` 按发布时间展示全部结果；过期消息折叠到单独区域，用户可展开查看。
- `feed-item-cover` 已支持真实图片，并继续使用懒加载和低优先级请求；旧站相对图片优先解析到 `https://yct.shangxiaoguan.top`，语义色变量名不再误当作图片 URL。
- 已处理：首页 Hero 首屏封面使用 `fetchPriority="high"`，信息流和过期消息列表封面使用懒加载，降低运营消息图片一次性加载对首屏和移动网络的影响。
...
- 新增 `/api/account/status`：读取 `ldpass` 会话并区分未配置、未登录、Active 登录、只读账号和会话不可用；只有匹配 YCT 本地管理员成员时才合并内容、服务入口、交通数据和 POI 的待审核计数。头像优先使用 `ldpass` 自定义头像，缺省时按服务器账号名或用户名回退到 `https://minotar.net/helm/<user>`，前端头像框已改为更适合 Minecraft 头像的圆角矩形。
...
- 登录相关接口已增加 `no-store` 缓存头，降低反代或浏览器缓存旧登录状态、旧 `state` 或旧账号状态的风险；`/api/auth/ldpass/client-session` 会额外返回 `yctDiagnostics`，用于区分 YCT 配置缺失、未收到 `ldpass_session` 共享 Cookie、以及收到 Cookie 但 `ldpass` 未返回 Active 用户三类问题。公网域名场景下会忽略反代误传的内部 `X-Forwarded-Port: 3300`，避免生成 `https://yct.shangxiaoguan.top:3300/...` 这类错误回跳地址。
...
- 已处理：`/admin/operations` 允许把已有 `draft` 或 `rejected` 内容重新载入编辑器继续修改；保存已驳回内容时会恢复为 `draft` 再进入后续提审流程，并发布 `ContentDraftUpdated` 事件用于审计和缓存刷新。
- 已处理：内容后台编辑器新增与前台详情同源的 Markdown 预览区，可同步预览分类、标题、摘要和正文，降低后台预览与前台渲染不一致的风险。
- 已处理第一版：内容后台的已通过内容可选择立即发布或定时发布；当前过渡实现使用未来 `publishedAt` 表示定时发布时间，公开侧会过滤未来时间的本地内容，避免“定时内容提前出现在首页”。后续接数据库和计划任务时再升级为正式调度状态机。
- 已处理第一版：内容后台编辑器已开放摘要、封面色、封面图链接和有效期元数据的编辑，并允许把草稿、待审核、已通过、已驳回或已发布内容归档下线；内容工作流新增 `ContentArchived` 事件用于审计和公开缓存刷新。
- 已处理第一版：内容后台新增重点资讯排序字段；前台首页 Hero 会优先选择 `showInBanner=true` 且排序值更小的内容，未填写排序时按发布时间回退。后台预览标题同样会把旧标题中的 `|` 作为软断行点处理，保持与前台一致。
- 已处理第一版：内容后台支持自定义标签录入，标签会进入本地内容仓储；前台内容详情可展示标签，站内搜索也会把标签文本纳入运营内容匹配。当前仍是自由文本标签，不含标签字典、别名和全局去重规则。
- 已处理第一版：首页强提醒规则已接入本地仓储 `.yct-data/operations-reminder-rule-store.json`、后台 API `/api/admin/operations/reminder-rules` 和首页公开读取逻辑；管理员可配置手动提醒或关联公开运营内容的提醒规则，设置启用状态、排序、标签、语义色、时间窗和跳转链接。首页运营页会只展示当前生效的规则，并在关联内容未公开或已过期时自动跳过，避免无效提醒卡片继续停留在首页。
- 已处理补充：旧 `ltcx/stop.txt` 解析出的 `TransitServiceNotice` 已并入首页强提醒候选与运营提醒投递预览，当前先自动生成“客运调整”类提醒并跳转到 `/travel/schedules`；这样旧客运班次调整可以不依赖手工再录一遍首页强提醒规则。后续如果要扩展到轮渡、航班或线路影响分群，可继续沿用同一候选汇总层。
- 已处理第一版：运营提醒已接入现有通知投递骨架。保存首页强提醒规则时会发布 `OperationsStrongReminderRulesUpdated` 事件，监听器会读取当前生效或未来生效的运营提醒规则，为已开启“运营提醒”通知类型的登录用户创建或刷新 `operations` 类型 Push 投递记录；存在活跃浏览器订阅的用户会进入 `queued`，无订阅用户记录为 `skipped`，失效规则对应的待投递记录会自动取消。当前登录用户修改通知偏好、登记浏览器订阅或撤销订阅时，也会自动重算该用户的运营提醒投递，并把已经失效的 `queued` / `deferred` / `skipped` 记录标记为 `cancelled`。当前仍未接入线路影响自动分群、人工确认发送、聚合去重策略和后台投递审计面板。
- 已处理补充：自动客运公告候选已经接入同一条运营提醒投递链路；`/admin/operations` 的“投递预览”与“重算投递”会一起覆盖手工规则、关联内容和旧 `ltcx/stop.txt` 公告。
- 已处理补充：新增内部同步工作流与受 `YCT_INTERNAL_TASK_TOKEN` 保护的 `/api/internal/operations/reminders/sync`。该入口会读取当前 `TransitServiceNotice` 列表、与 `.yct-data/operations-reminder-source-state.json` 中保存的上次签名做比较；一旦发现旧客运公告源发生变化，就会发布 `OperationsReminderDeliveryRefreshRequested` 领域事件，并由现有通知监听器接手后续运营提醒投递重算。下一步只需把这个入口挂到正式定时任务，即可把“自动客运公告候选”从手动重算推进到半自动同步。
- 已处理补充：如果首页强提醒规则绑定的是某条运营内容，当前该内容在后台发布或从 `published` 归档时，也会由专门监听器发布 `OperationsReminderDeliveryRefreshRequested` 事件，自动重算首页强提醒候选与 `operations` Push 投递，避免内容状态变化后仍停留旧提醒。
- 已处理补充：统一内部任务 runner `/api/internal/tasks/run` 已默认纳入客运公告源同步；同一轮任务会依次执行“公告源变更探测 -> 事件 Outbox 重放 -> 到期 Push 投递处理 -> 过期票务占座清理”。同时新增 `scripts/run-web-internal-tasks.ps1` / `pnpm web:tasks:run` 作为固定调用脚本，便于本地和云端计划任务统一入口。
- 已处理补充：`/admin/operations` 的运营提醒预览区已能展示旧客运公告源的同步状态，并提供“同步公告源”“强制同步并重算”按钮，方便在不登录服务器的情况下验证当前实例是否读到了最新 `ltcx/stop.txt`。
- 已处理补充：`/admin/operations` 现在还可直接运行统一内部任务 runner，并回显最近一轮“公告同步 / 事件重放 / 通知处理 / 票务清理”结果摘要，测试和排障时不必总是切回服务器终端。
- 已处理补充：最近一轮统一内部任务结果已持久化到本地仓储，后台刷新后也能继续查看，不再依赖刚好保留上一次接口响应。
- 已处理：`/admin/operations` 的运营提醒投递预览区新增“重算投递”按钮，后台会发布 `OperationsReminderDeliveryRefreshRequested` 事件并立刻重新读取当前候选、用户偏好和设备订阅，便于管理员在不改规则正文的情况下手动触发一次全量运营提醒投递重算。
...
- `/account` 新增本地历史概览，展示行程提醒、历史行程、班次查询记录、地图收藏和待同步数量，提供进入出行页管理、班次查询、地图页、同步提醒到账号和清空新版本地历史的入口。
...
- 当前投稿接口要求登录 `ldpass` 且已验证服务器账号；匿名用户、非验证用户和私有 POI 暂不开放提交。图片第一版支持用户文件上传到 `.yct-data/poi-submission-images`，上传成功后发布 `PoiSubmissionImageUploaded` 事件并把生成的图片 URL 写入 POI 投稿；前台公开 POI 投稿表单已新增地图选点按钮，可直接从地图空白处回填点坐标。图片安全检查、对象存储、营业时间、电话、出入口/设施关联、代表 POI、线性/区域几何的可视化编辑仍待后续后台与表单设计；后台中涉及坐标、站点和线路途径点的字段也应继续复用地图选点控件，并支持按特定分类筛选可选标记。
...
- 管理员 PIN 来源。
- 票务库存、核销链接、退票和对账细则。
- 已确认补充：HTTP 源只负责实时瓦片图和玩家位置，地点标记与精细静态瓦片图继续来自 `map.shangxiaoguan.top`；HTTP 瓦片图、玩家标记等动态地图数据通过 YCT 后端代理处理，瓦片图也可以优先使用现有静态 HTTPS 源作为安全默认源。较新 HTTP 瓦片的首版代理已补到 `/api/map/tile-proxy`，地图图层面板已支持在多个瓦片 Provider 间切换。
- 已确认补充：后台标记数据转道路图采用手工连接和自动抽取结合；线路型 POI 可按需拆出方向子 POI，途径坐标由线路决定是手工填写还是遵守道路级规划生成。
- 已确认补充：面性地点兼容多个矩形和多边形两种方式，显示层转换为多边形或合并轮廓展示。
- 已确认补充：内容审核暂时不需要自动敏感词、外链提示或图片安全自动处理；预览环境通过后台嵌入和预览 URL 实现。
...
- 已处理第一版：新增 `GET /api/travel/ticketing/orders/:orderId` 当前用户订单详情读取入口，以及 `/travel/ticketing/orders/[orderId]` 二级详情页。详情页只显示订单自身、完整订单 ID、库存占用 ID、库存池、状态和时间戳，并保留取消草稿操作；当前不展示支付、票券、条形码或核销凭证，避免把草稿订单误认为真实电子票。
- 已处理第一版：顶部账号徽标已把当前 Active 用户的待处理票务订单数纳入合并计数，统计范围为 `draft`、`pending_issue` 和 `manual_review`；管理员待办仍只对管理员账号计算和返回。
- 已处理第一版：票务订单创建或取消成功后，前端会派发本地订单状态变更事件，顶部账号入口会重新读取 `/api/account/status`，让合并徽标及时反映待处理票务订单变化。
- 已处理第一版：票务订单工作流在列表、详情、计数和创建草稿前会清理过期占座；已过期的 `draft` 订单会回写为 `expired`，库存占用回写为 `expired` 并发布系统触发的 `TicketOrderCancelled` 事件，原因使用 `inventory_expired`。内部计划任务 runner 也会主动执行这项清理并返回清理统计。
...
- 已处理第一版：旧地图标记点中“道路”分类下所有同名标记点会在 `/api/map/markers` 额外归并为 `MultiPoint` 端点集合；这些端点没有严格顺序，地图侧栏可搜索和定位，瓦片关闭或路网相关浏览状态下可绘制旧端点排序生成的粗线近似轨迹；该轨迹仅用于视觉识别道路，不代表正式道路级导航。
- 新增待办：线路需要作为一类或两类独立线性 POI 存进数据库；数据至少记录线路方向、途径站点点标记和展示属性，可选记录途径坐标序列。若不记录坐标，则必须通过道路级导航按途径站点规划路线，不能直接用站点顺序生成伪线形。
- 已处理第一版：`/map` 搜索或选中有真实坐标的线性 POI 时，会在地图上展示中心标签和端点小点；道路端点组可展示粗线近似轨迹但明确不作为可导航路线，`transit-line` 在未补线路坐标前仍只作为侧栏检索和详情入口，避免把无序端点或缺失坐标伪装成正式路线。
...
- 已处理：地图标记点文本不再尝试省略；碰撞或空间不足时应隐藏标签或标记，而不是显示省略号截断后的地名。碰撞盒估算也已按完整文本长度计算，不再沿用 120px 的旧截断宽度。
- 已处理第一版：点击地图标记或侧栏选中特定兴趣点后，会在地图操作栈中展示“简介 / 设施或出入口”详情面板，并在地图上高亮该兴趣点；周边搜索移入操作栏按钮，当前设施数据仍等待后台结构化补充。
- 已处理第一版：公交、地铁、有轨、客运、地方铁路等线路介绍已有地图探索二级页面 `/map/lines/[id]`；`/travel` 已收窄为“出行服务”和“行程提醒”两个模块，出行服务只保留班次查询、线路与站点地图导向、智运大屏/客运展示、票券与订单预留入口，完整线路浏览继续交给地图探索。真实订票、票券、核销、退票以及轮渡/航班更深接入仍待后续统一票务平台推进。
...
- 已处理第一版：普通点状地点详情增加操作栏，“查看路线”使用 `directions` 图标和重点按钮样式，“搜索周边 / 收藏 / 分享”使用图标按钮；道路、线路等线性地点不显示普通地点操作栏。搜索周边、查看路线、浏览器本地收藏和分享/复制地点链接已具备第一版行为，地图标记列表可用“收藏”分类筛选本机收藏地点；登录用户进入账号页时会通过 `/api/account/map-favorites` 合并本机和账号侧 markerId 收藏，服务端暂存到 `.yct-data/map-favorite-store.json` 并发布 `MapFavoritesUpdated` 事件。
- 已处理第一版：地图缩放改为交互层连续无级缩放，瓦片资源层仍按最近的 uNmINeD 整数 zoom 层级取图并进行屏幕缩放；比例尺、标记点和轨迹使用连续缩放值计算。
- 已处理第一版：`/travel` 主内容不再铺开完整线路列表，只保留“出行服务”和“行程提醒”两个主模块；客运班次、统一票务平台预留、客运大屏、线路与站点导向都收纳到出行服务内。当前出行服务卡片包含班次查询、线路与站点地图入口、智运大屏和票务预留，完整线路浏览继续转入地图探索。
...
- 已处理第一版：统一班次查询新增浏览器本地班次记录，保存到 `yct.travelScheduleHistory.v1`，最多保留最近 50 条；用户可手动保存班次，添加班次提醒时也会自动记录。账号页“本地历史”会合并展示行程提醒、班次记录和地图收藏，但这些记录不代表订单、票券、占座、核销凭证或账号收藏。
...
- 已处理第一版：地铁线路作为地图底图覆盖层在卫星、路网、交通三种模式下常驻显示，线条使用对应线路色；路线规划结果态不隐藏地铁底图线，规划路线叠加在其上。线路没有单独途径坐标时，先按已定位站点坐标和站序直线连接；当旧交通站点表缺少 Minecraft 坐标时，地图 API 会按站名匹配静态地图里的站点 POI 坐标生成折线，后续可用后台维护的精确坐标替换。
- 已处理第一版：地图内线路详情的站点时间线节点/线条使用线路色或交通方式语义色；站点列表提供正向/反向分段控制，方向切换会根据 `oneWay: up/down` 过滤单向站，避免简单反转完整站点数组。
- 已处理：旧线路数据中的 `oneWay: down` 表示与数据记载方向相同，`oneWay: up` 表示与数据记载方向相反；地图内线路详情、普通线路详情和路线规划候选共用的方向过滤已按这个语义修正。
...
- 已处理第一版：地图页支持通过 `marker`、`line`、`lineId` URL 参数聚焦地点或线路对象，兼容 `transit-line-<id>` 与裸线路 ID；同一地图页内参数变化时也会重新聚焦，便于搜索页、分享链接和后续深链入口直接打开地图详情。

2026-07-03 当前 3300 预览新增反馈，先记录为待办：

- 已处理第一版：地图页面支持触控设备双指捏合缩放，复用现有连续缩放和地图中心换算，瓦片层、标记点层、道路/线路轨迹层继续跟随同一套 `mapView` 状态同步缩放。
- 已处理第一版：区域/父子 POI 先按旧地图命名规则推导只读关联；若存在父地点 `XXXX`，则点名 `XXXX-OOOO` 会出现在父地点详情的“设施/出入口”栏中，并可点按聚焦到对应子 POI。设施页签会先按名称和分类启发式分为出入口、交通、楼栋、景点、设施、周边等组；存在子地点的父地点会作为临时默认代表点参与地图碰撞优先级加权，优先保障这类代表地点展示。当前已按旧图标语义收窄出入口判定：`exit*` 图标仅作为地铁站出入口，普通地点出入口优先使用 `way-in` / `way-out`，公交站点不再作为父 POI 吸收子地点；从子地点出发或到达子地点时，若所属父地点存在出入口子地点，路线规划会先经父地点出入口再接入道路或公共交通。后续仍需要后台显式维护出入口、楼栋、景点、设施等分组，并支持区域 POI 指定或覆盖代表 POI。
- 路线规划需要从“路线草稿卡片”升级为路线规划页能力：起点/终点先按自身或上级区域 POI 的入口/出口吸附到道路，再在道路网络中规划；道路之间当前按约 100 格范围内的连通/交叉候选处理；公共交通启用时需要把步行接驳、站点间公共交通线路和终点接驳组合成完整路径。
- 已处理第一版：地图路线规划卡片新增交通方式开关和多方案初步结果；当前根据直线距离、旧道路端点、已导入站点、真实线路站序和站点 `travelTime` 生成步行直达、公共交通直达和一次换乘候选，按耗时/距离排序后根据启用交通方式数量动态展示多条可能路径，并在地图上用虚线折线高亮选中方案。步行与公交/客运接驳已优先把起终点投影到更贴近目标方向的最近道路段，再在旧道路端点图中按 100 格连通候选拼接过渡路径；道路间连通候选改为按线段到线段最近点生成，不再只看道路标记点的点到点距离；道路接入候选会同时保留综合方向评分较优和距离地点最近的投影点，减少从路口或拐点过早切直线的情况；无法连通的片段才回退直线估算，并明确标注不代表正式道路级导航。路线卡片已增加“最快到达”“最少换乘”“最少步行”等徽标；步骤时间线已从纯文本升级为地点、步行、乘车和换乘过程的结构化节点，并按参考稿调整为地点节点与过程竖条的视觉层级，乘车步骤使用对应线路色或交通方式语义色。`is-walk`、`is-transfer` 与 `is-transit` 步骤已增加可展开详情：步行和换乘步行可展示按道路分组的距离、估算耗时和粗略转向图标，公共交通可展示经过站点。地图路线高亮已拆分步行、换乘步行和每段公共交通线路：步行段改用页面强调色以提升在地图底图上的可读性，每条线路段使用自身线路色或交通方式语义色。路线结果态保留的起点、终点和途经站点从未碰撞过滤的投影点中选取，不再被地图碰撞隐藏规则误删。公共交通候选会过滤起终点同站进出的结果，线路区间切片使用过滤后的可定位站点索引，避免把实际乘坐区间外的终点站或无关站点纳入地图高亮路径；线路展示和站点匹配会优先约束到同类别站点，尽量避免地铁线误连公交站、铁路站等异类站点。路线候选计算已从 render 过程中拆出，首次规划前会缓存旧道路图并显示“正在规划”的提示，避免路线卡片在大路网下无反馈卡顿；同一轮规划中的道路接驳候选、节点路径和坐标对道路结果也会复用缓存，减少公交/换乘候选反复计算同一段路。区域/父子 POI 已按旧命名规则加入临时接驳：优先使用入口、出口、出入口、站口、门等子地点；如果没有这类子地点，则用同方向最近子地点兜底。地铁进出站第一版会优先经过地铁站出入口；地铁线路之间换乘保持站内接驳，不使用出入口绕行。路线规划卡片已支持分享深链接，链接会携带起终点、交通方式和当前候选方案参数，打开后可恢复草稿并重新计算；分享反馈已改为短暂全页面 toast，后续仍要补“分享链接 / 文字版 / 分享图”的选择式分享流程。道路图构建已补充道路尽头到另一条道路线段投影点不足 100 格时的临时连通规则；实际相交仍按线段交点连通，非相交的近距离连通只从道路尽头投影到另一条道路线段，避免把普通拐点控制点误判为可跨越路口。所有方案均明确标注为估算方案，正式道路图、后台可编辑出入口/方向规则、线路真实轨迹坐标、高阶换乘和多策略权重仍待道路网络、线路方向坐标和时刻表数据继续完善。
...
- 已处理第一版：子 POI 详情页会显示所属父 POI，并可点按返回父地点；子 POI 的“设施/出入口”页签也会展示所属父地点入口，避免切换页签后丢失父子关系。全局搜索页的线路结果改为打开地图并聚焦对应线路 marker；道路和线路详情复用 POI 操作栏，支持查看路线、搜索周边、收藏和分享。
...
- 已处理第一版：`/travel/schedules` 的 `schedule-trip-card` 参考用户 2026-07-03 附图重排，顶部显示交通方式图标、真实班次号/线路和票价，主体强调出发时间、运行/经停和到达时间，移动端继续保持三段式票据布局；到达时间的跨日偏移会使用上标标记而不是跟随主时间字号放大。底部保留添加提醒、保存记录、旧版参考和票务状态操作，运营方和车型/船型/机型字段仍只展示真实数据或待公布状态。
- 已处理第一版：`/account` 通知与免打扰区域文案拆清楚，总开关控制本设备是否接收推送，分类决定允许哪些类型推送，免打扰时段只负责静默或延后提醒。
- 已处理第一版：动态效果设置接入 design token；`duration-fast`、`duration-standard` 和新补充的 `duration-spinner` 会响应用户的动态效果偏好和系统 `prefers-reduced-motion`。路线规划等待状态使用 spinner token，并在关闭动态时禁用持续旋转。
- 已处理第一版：针对界面文字过度加粗的问题，先收敛顶部页标题、地图路线卡片、路线步骤时间线、POI 关联项和票务详情等紧凑 UI 的过重 `font-weight`，保留英雄标题、关键数字和状态徽标的必要强调；后续视觉回归时继续按页面逐块检查。
- 已处理规划第一版：新增 `docs/I18N_PLAN.md`，明确多语言第一阶段支持 `zh-CN`、`zh-Hant` 和 `en`，固定界面文案走翻译目录，地名、组织名、站名等业务数据必须走正式译名字段、别名表、后台维护或审核投稿，不能先用机器翻译直接替换。语言切换控件中的语言名称固定使用自称名称：`简体中文`、`繁體中文`、`English`。契约层已补 `LocalePreferenceUpdated`、`TranslationCatalogPublished` 和 `EntityTranslationUpdated` 事件，后续用于账号偏好同步、翻译目录发布、实体译名审核和搜索索引刷新。客户端已新增 `common` 翻译目录与 `useI18n` hook，并先接入主导航、顶部搜索/乘车码/账号入口、普通用户二级页返回按钮与固定标题、账号页外观/语言/字体/动态偏好控件、搜索页固定文案、离线页固定文案、首页运营信息固定文案、更多服务页固定文案、出行一级页固定文案、行程提醒面板标题/摘要/弹窗表单/列表空态/状态标签/来源标签/操作按钮固定文案、统一班次查询页标题/筛选/本地记录/提醒公告/班次卡片基础操作/票务按钮固定文案、站点详情页层级/出入口/设施/换乘周边/数据来源固定文案、线路详情页概览/方向/站点列表/单向停靠/站点摘要/数据来源固定文案、票务草稿面板、票务订单详情页、订单状态/服务类型/取消原因/库存占用状态固定文案，以及地图页标题、搜索框、标记列表空态、附近搜索提示、POI 常用操作、已知标记分类名、已知交通方式名、路线规划卡片端点编辑/交通方式/候选方案/步骤详情/估算说明固定文案、图层与投稿面板、地图数据状态提示、地图比例尺/坐标提示和公开 POI 投稿弹窗固定文案。
