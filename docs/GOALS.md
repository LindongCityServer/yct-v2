...
- 已处理第一版：区域/父子 POI 先按旧地图命名规则推导只读关联；若存在父地点 `XXXX`，则点名 `XXXX-OOOO` 会出现在父地点详情的“设施/出入口”栏中，并可点按聚焦到对应子 POI。后续仍需要后台显式维护出入口、楼栋、景点、设施等分组，支持区域 POI 指定代表 POI，并让标记碰撞检测优先保障代表 POI 展示。
