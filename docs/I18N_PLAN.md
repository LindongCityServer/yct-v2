...
3. UI 文案目录：先迁移低风险公共组件和设置页，再扩展到地图、出行和后台。当前已新增客户端 `common` 翻译目录和 `useI18n` hook，并先接入主导航、顶部搜索/乘车码/账号入口、普通用户二级页返回按钮与固定标题、账号页外观/语言/字体/动态偏好控件、搜索页固定文案和离线页固定文案；业务实体名称仍保持主语言回退，不做机器翻译。
