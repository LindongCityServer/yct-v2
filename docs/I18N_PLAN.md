...
2. 客户端偏好：账号设置页增加语言偏好控件，匿名用户写本地，登录用户同步账号侧。当前已完成登录用户服务端偏好 API、事件发布、客户端本地偏好工具和账号页语言偏好控件；全局 `PreferenceBridge` 会在页面进入时应用本地语言偏好、同步账号侧偏好、更新页面 `lang`，并通过轻量前端事件通知已接入 `useI18n` 的客户端 UI 更新。
3. UI 文案目录：先迁移低风险公共组件和设置页，再扩展到地图、出行和后台。当前已新增客户端 `common` 翻译目录和 `useI18n` hook，并先接入主导航、顶部搜索/乘车码/账号入口、普通用户二级页返回按钮与固定标题、账号页外观/语言/字体/动态偏好控件、账号身份/登录状态/账号入口操作、推送通知与免打扰设置、PWA 安装与缓存管理、自定义离线范围列表与弹窗、本地历史统计/同步/清理操作、搜索页固定文案、离线页固定文案、首页运营信息固定文案、更多服务页固定文案、出行一级页固定文案、行程提醒面板标题/摘要/弹窗表单/列表空态/状态标签/来源标签/操作按钮固定文案、统一班次查询页标题/筛选/本地记录/提醒公告/班次卡片基础操作/票务按钮/跨日偏移固定文案、客运大屏详情页筛选/摘要/空态/班次元信息固定文案、站点详情页层级/出入口/设施/换乘周边/数据来源固定文案、线路详情页和地图内线路详情的概览/方向/站点列表/单向停靠/站点摘要/数据来源固定文案、账号页票务草稿入口和票务草稿面板、票务订单详情页、订单状态/服务类型/取消原因/库存占用状态固定文案，以及地图页标题、搜索框、标记列表空态、附近搜索提示、POI 常用操作、POI 详情属性/接驳线路/关联地点分组固定文案、已知标记分类名、已知交通方式名、图层与投稿面板、地图数据状态、比例尺/坐标 HUD、公开 POI 投稿弹窗和路线规划卡片端点编辑/交通方式/候选方案/步骤详情/估算说明固定文案；业务实体名称和未知服务器自定义分类仍保持主语言或原始名称回退，不做机器翻译。瓦片源这类系统提供的可见名称需要按语言目录本地化，`临东` 在英文界面统一写作 `Lindong`。
