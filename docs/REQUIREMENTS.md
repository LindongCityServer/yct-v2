...
- 当前工程补充：道路图和点状标记的道路投影索引会在地图数据阶段预先构建；每个标记点按道路只保留一个最近投影，最近投影总是可用，其它道路投影必须在标记点 50 格范围内。路线端点、出入口、公交/客运站点之间的道路段会优先使用这些固定投影，减少斜穿街区。父地点与子地点互相规划时不强制道路投影，出入口类子地点不会自动改走同父地点的其它出入口。路线交通方式开关采用短延迟批量重算；路线方案摘要时间按已生成步行/道路接驳段估算，避免比详情时间明显偏短。
- 当前工程补充：地图数据和瓦片可见后，如果道路图与标记点道路接入点仍在计算，地图区域需要显示状态提示；路线规划应等待接入索引 ready 后再生成候选，避免在索引未完成时先落入过度直线化的临时结果。
- 当前工程补充：地图地点与路线支持选择式分享，至少包含复制链接、复制文字版和生成分享图。路线分享只描述当前展示的候选方案；文字版可使用箭头、圆点等符号表达起终点、步行、乘车和换乘；分享图应复用路线卡片的 `map-route-step-timeline` 视觉结构，步行段默认展示 `map-route-step-detail-list`。当前前端已使用 `ms` / `rs` 紧凑编码缩短分享链接并兼容旧参数；如果继续采用前端携带状态的方式，应先压缩分享负载再做 Base64URL 编码，避免长地名和复杂路线造成链接膨胀；若需要进一步显著缩短，应新增服务端短链存储，用短 slug 解析分享状态。
