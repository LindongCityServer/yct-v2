...
- 实时玩家位置：通过 BDSLM HTTP 源获取坐标和名称，但前端第一阶段使用 Material Symbols 符号标记展示，并以“在线玩家”作为展示分类；不把 HTTP 源返回的相对图片名当作地点图标文件使用。
...
- Banner / Hero：展示重点图文资讯，可跳转站内内容页或外部链接；Hero 封面使用真实图片元素承载首屏重点图，并叠加从下往上的遮罩渐变，便于标题在底部保持可读。
...
- 当前工程状态：首页 Hero 封面已从 CSS `background-image` 改为真实 `<img>`，首屏重点资讯使用 `fetchPriority="high"`；信息流和过期消息列表中的封面继续使用懒加载和低优先级请求，避免运营消息图片一次性全部抢占带宽。

后台能力：

- 内容新建、Markdown 编辑、预览、提交审核、通过、驳回、定时发布、下线。
- 当前工程状态：内容后台的 Markdown 预览已复用前台运营详情同一套白名单渲染组件，标题、摘要和正文可以在编辑区旁边即时预览，避免后台预览与前台展示规则分叉。
- 当前工程状态：内容后台第一版已支持“立即发布”与“定时发布”两种入口；定时发布暂以未来 `publishedAt` 表示，公开前台在读取本地内容仓储时会隐藏未来发布时间的记录，待时间到达后再对外可见。后续接数据库与任务调度后，应改为更明确的定时发布状态与计划任务模型。
- 当前工程状态：内容后台已开放摘要、封面色、封面图链接和有效期字段的编辑，并支持把任意未归档内容归档下线。当前站内封面图既可填写完整 URL，也可填写 `/content-assets/...`、`/legacy-assets/...` 等站内公开路径。
...
- 草稿继续编辑：后台需要允许把已有 `draft` 或 `rejected` 内容重新载入编辑器。保存驳回内容时，应回到 `draft` 状态后再重新提交审核，不能直接保留 `rejected` 状态覆盖正文。
- 旧资源下载：清单中的下载候选可以先落盘到 `apps/web/public/legacy-assets` 并生成校验报告；该步骤只保证资源不丢失，不代表素材已通过管理员审核。
- Banner 是否展示、排序、有效期管理。
- 当前工程状态：内容后台第一版已支持“作为首页重点资讯候选”和“重点排序”两个字段；数值越小越优先成为首页 Hero，未填写时按发布时间回退排序。旧标题中的 `|` 仍按软断行点处理，后台预览与前台展示保持一致。
- 分类和标签管理。
- 当前工程状态：内容后台第一版已支持自定义标签录入，使用每行一个或逗号分隔的纯文本标签；标签会写入本地内容仓储，前台内容详情展示标签，站内搜索也会将标签文本纳入匹配。当前仍未提供独立标签字典、标签别名或管理员级标签去重策略，后续如需跨内容复用再升级为正式标签管理。
- 首页强提醒规则管理。
- 当前工程状态：首页强提醒规则第一版已接入本地仓储 `.yct-data/operations-reminder-rule-store.json`、后台管理入口 `/api/admin/operations/reminder-rules` 和公开读取逻辑。管理员当前可配置手动提醒与关联运营内容两种规则来源，设置启用状态、排序、标签、语义色、时间窗和跳转链接；此外系统会自动把旧 `ltcx/stop.txt` 解析出的 `TransitServiceNotice` 作为“客运调整”强提醒候选并入首页与运营提醒预览。首页强提醒面板只展示当前生效、内容已公开且未过期的候选；客运公告候选不需要额外手工录入，但仍受开始/结束时间过滤。后续再补线路调整自动命中、行程影响联动、推送编排和更细的可视化规则编辑。
- 当前工程状态：运营提醒 Push 第一版已接入现有事件总线与投递仓储。后台保存首页强提醒规则时会发布 `OperationsStrongReminderRulesUpdated` 事件，并按当前启用了“运营提醒”通知类型的登录用户生成或刷新 `operations` 类型 Push 投递记录；有活跃浏览器订阅的用户会进入 `queued`，无订阅的用户会记录为 `skipped`。当前登录用户修改通知偏好、登记浏览器订阅或撤销订阅时，也会自动重算该用户的运营提醒投递：失效的 `queued` / `deferred` / `skipped` 记录会标记为 `cancelled`，再按最新偏好与订阅状态补写新的投递结果。自动并入的客运公告候选同样会参与投递计算；当前还新增了受 `YCT_INTERNAL_TASK_TOKEN` 保护的内部入口 `/api/internal/operations/reminders/sync`，用于轮询旧 `ltcx/stop.txt` 公告源、比较签名并在发现变化时发布 `OperationsReminderDeliveryRefreshRequested` 事件，从而复用现有重算链路。被强提醒规则引用的运营内容在发布或从 `published` 归档时，也会通过事件监听器自动发布同一事件，避免首页重点提醒和 Push 候选长期停留在旧状态。正式定时任务部署、线路影响自动命中、按提醒范围分群、二次确认发送和批量节流面板仍待后续实现。
- 当前工程状态：统一内部任务 runner `/api/internal/tasks/run` 已默认把“客运公告源同步”纳入同一轮计划任务，执行顺序为：客运公告源签名比较与必要的运营提醒重算请求 -> 事件 Outbox 重放 -> 到期 Push 投递处理 -> 过期票务占座清理。仓库与部署包均附带 `scripts/run-web-internal-tasks.ps1` / `pnpm web:tasks:run` 作为固定调用脚本，方便在本地与云端统一调度。
- 当前工程状态：内容后台“运营提醒投递预览”已支持查看提醒候选、目标用户、最近投递记录，并提供“重算投递”按钮。该按钮会发布 `OperationsReminderDeliveryRefreshRequested` 事件，强制重新计算当前全量运营提醒投递，适合测试或在偏好、订阅与规则数据疑似不同步时排障。后台同区块还会展示“旧客运公告源”的最近检查时间、变化时间和最近请求重算时间，并提供“同步公告源”“强制同步并重算”按钮，便于人工验证旧 `ltcx/stop.txt` 是否已经被当前实例读取到。
- 当前工程状态：后台同区块还可直接运行统一内部任务 runner，手动触发“公告源同步 -> 事件 Outbox 重放 -> 到期通知处理 -> 票务过期清理”一整轮流程，并回显最近一轮结果摘要，方便不登录服务器时做联调与排障。
- 当前工程状态：最近一轮统一内部任务结果已落到 `.yct-data/internal-task-run-store.json`，因此后台刷新后仍能看到上一次运行的处理时间、公告同步状态、事件重放数、通知处理数和票务清理数，不依赖页面必须命中那次响应。
- 当前工程状态：同一仓储当前会保留最近几轮轻量任务摘要，用于后台回看近几次“系统任务/管理员手动任务”的执行情况；历史记录只保留聚合计数与状态，不保留完整通知明细，避免本地 JSON 持续膨胀。
- 当前工程状态：后台会基于最近一轮统一任务结果给出保守的任务级状态摘要；当公告源不可用/未配置、事件重放失败数大于 0，或通知失败数大于 0 时标为“有告警”，其余情况标为“正常”。第一版不做更细的失败分类面板，后续如需深挖再扩展。
...
- 路线规划吸附规则：标记点通常不会精确落在道路上，起点和终点应先吸附到最近道路；如果 POI 属于区域 POI，应优先使用其绑定的出口/出入口/入口点中距离最近且方向合适的点作为接驳点。起点到道路、道路到终点之间可以生成直线步行接驳段，但需要明确与道路网络段区分。当前工程第一版已按“地点到道路线段投影点、投影点进入道路图、离开道路图后再到终点”的方式计算步行接驳；若道路图候选无法连通才回退直线估算。
- 道路网络连通规则：从一条道路转向另一条道路时，后台道路图应能表达交叉、连接点和可通行规则；在只有旧道路端点数据的过渡阶段，可把约 100 格范围内经过的其他道路视作连通或交叉候选，但正式路线规划必须保留可解释的节点、边、权重和来源，不能把视觉粗线直接当作导航图。
- 公共交通路线规划：启用公共交通时，路线应由起点步行接驳、公共交通站点间线路段、换乘/步行段和终点接驳组合而成；公共交通线路段必须使用真实线路、站点和方向数据，不能凭站名顺序或近似轨迹伪造可乘坐路线。
- 当前工程状态：地图 POI 详情中的“查看路线”已能创建路线规划草稿，并生成第一版估算路线结果。步行方案保留直线估算，并新增基于旧道路端点近似排序与约 100 格连通候选的“道路步行估算”；该方案只作为过渡预览，不宣称为正式道路级导航。公共交通方案会按启用的交通方式遍历真实线路和方向，使用线路站序、站点标记和旧数据中的 `travelTime` 生成直达或一次换乘候选，排序后按启用方式数量动态展示多条可能路径，并在地图上高亮。路线卡片会标记“最快到达”“最少换乘”“最少步行”等候选特征，步骤时间线已先按地点、步行、乘车和换乘过程区分样式，乘车过程按对应线路色或交通方式语义色显示；计算中会显示轻量等待状态和“首次构建道路图可能需要几秒”的提示，避免用户误以为界面无响应。路线结果态会隐藏非路线相关标记，仅保留起终点和经过的站点标记；这些路线相关标记从未碰撞过滤的投影点中选取，不再被地图拥挤碰撞规则隐藏。区域/父子 POI 已能在规划计算中把父地点按旧命名规则临时接驳到子地点：旧图标 `exit*` 只在父地点为地铁站时作为地铁出入口，普通地点优先使用 `way-in` / `way-out` 图标作为出入口；没有图标的手工数据保留名称兜底。如果没有这类子地点，则用朝向另一个端点最近的子地点兜底，并把这段接驳计入距离、时间和路线高亮。公交站作为点状站点不再作为父 POI 吸收 `XXXX-OOOO` 子地点。起点或终点本身是子地点时，如果所属父地点存在出入口子地点，路线也会先经过父地点出入口再接入道路或公共交通。地铁进出站接驳会在有出入口时经过地铁站出入口；地铁线路之间换乘保持站内站点接驳，不绕出入口。道路接入候选会同时保留方向评分较优和距离地点最近的投影点，降低从路口或拐点过早切直线的概率。路线规划卡片支持分享，链接会携带起终点坐标、标签、端点 markerId、启用交通方式和当前候选方案，打开后恢复路线草稿并重新计算候选；分享反馈使用短暂全页面 toast，不改变路线卡片布局。地图标记列表的分类筛选已包含“收藏”，可按浏览器本地收藏的地点 ID 过滤列表。过渡道路图构建已补充道路尽头到另一条道路线段投影点不足 100 格时的临时连通规则；真实线段相交仍按交点连通，非相交近距离连通只从道路尽头投影到另一条道路线段，避免把普通拐点控制点当作可跨越路口。正式道路图、后台可编辑出入口/方向规则、线路轨迹坐标和更完整换乘权重仍待后续接入。
- 当前工程补充：道路图和点状标记的道路投影索引会在地图数据阶段预先构建；每个标记点按道路保留最近投影，其中距离最近的投影默认保留，其它投影仍需在标记点 50 格范围内。同一个投影坐标如果被多个标记点候选命中，只归属于距离该投影点最近的那个标记点，避免道路尽头端点被多个地点同时占用。路线端点、出入口、公交/客运站点之间的道路段会优先使用这些固定投影，减少斜穿街区。父地点与子地点互相规划时不强制道路投影，出入口类子地点不会自动改走同父地点的其它出入口。路线交通方式开关采用短延迟批量重算；路线方案摘要时间按已生成步行/道路接驳段估算，避免比详情时间明显偏短。
- 当前工程补充：地图数据和瓦片可见后，如果道路图与标记点道路接入点仍在计算，地图区域需要显示状态提示；路线规划应等待接入索引 ready 后再生成候选，避免在索引未完成时先落入过度直线化的临时结果。
- 当前工程补充：地图地点与路线支持选择式分享，至少包含复制链接、复制文字版和生成分享图。路线分享只描述当前展示的候选方案；文字版可使用箭头、圆点等符号表达起终点、步行、乘车和换乘；分享图应复用路线卡片的 `map-route-step-timeline` 视觉结构，步行段默认展示 `map-route-step-detail-list`。当前前端已使用 `ms` 紧凑编码地点分享；路线分享会在 `rsc` 压缩编码和旧 `rs` Base64URL 紧凑参数之间自动选择更短的参数，并继续兼容旧链接；若需要进一步显著缩短，应新增服务端短链存储，用短 slug 解析分享状态。
...
- 标记展示：地图标记默认缩小约 30%；小尺寸视口也保留文字标签。标记点文本不应为了避让而做省略号截断；如果当前尺寸和范围下图标或标签会互相重叠，应隐藏对应标记点或标签，不能用互相覆盖或被截断的文本硬挤。
...
- 区域 POI 关联：区域/面性 POI 需要能关联多个点状 POI，并按出入口、设施、楼栋、景点等分组展示在详情的“设施或出入口”区域。区域 POI 可以指定代表 POI，默认代表可按同名点标记推导；地图标记碰撞检测应优先保留代表 POI 的图标和标签。当前工程第一版先在前端按旧地图命名规则推导只读关联：若存在父地点 `XXXX`，则 `XXXX-OOOO` 会在父地点详情的“设施/出入口”中显示为子地点入口，并支持点按聚焦子 POI；子地点详情的“简介”和“设施/出入口”中都会展示所属父地点入口，点按可回到父 POI；展示层会按名称和分类启发式归入出入口、交通、楼栋、景点、设施、周边等分组；同时会把这些存在子地点的父地点作为临时默认代表点，提高地图碰撞检测中的保留优先级。后台显式分组、显式代表点和编辑能力仍待后续实现。
- 区域 POI 标签（待实现）：本规则只适用于 `Rectangle`、`MultiRectangle`、`Polygon` 和 `MultiPolygon` 等面性 POI，不包含 `LineString`。区域标签总体复用普通点状 POI 的文字、选中、点击和聚焦样式。正常状态只显示文字标签，不显示分类图标；标签与点状、线状 POI 一同进入同一套屏幕空间碰撞和视口越界判定。发生碰撞时，除当前已聚焦对象外，区域 POI 标签优先于其他非区域 POI 标签或标记保留；区域 POI 之间继续按既有业务优先级和稳定 ID 顺序确定保留项，避免每次渲染结果抖动。
- 区域 POI 标签越界降级（待实现）：每次平移或缩放后，应使用标签的完整屏幕碰撞框判断它是否完全位于对应区域的投影范围内；多区域几何只需完整落入其中一个有效区域，带孔多边形不得把孔洞视为可容纳标签的区域。若文字标签超出区域范围，则该标签在本次投影中转换为普通代表点 POI 标记，恢复分类图标和文字，并使图标中心严格落在区域 POI 的代表点坐标上；转换后的标记仍保持区域 POI 的碰撞保留优先级。该状态只由当前投影派生，不得写回 POI 数据；重新缩放或平移后满足完整包含条件时，应自动恢复为无图标的纯文字区域标签。
...
- 图层与投稿：`map-layer-panel` 第一阶段保留浏览模式、标记点/线条显示开关和投稿 POI 的弹窗入口；完整图层列表和常驻投稿表单后续再展开。公开 POI 投稿表单已支持“在地图上选点”回填点坐标；后台 POI 审核列表第一版已提供代表坐标、地图查看、复制坐标和复制几何 JSON 的辅助入口，便于管理员审核未发布投稿。后台中涉及坐标、站点、线路途径点和几何编辑的字段后续仍应复用地图选点能力，并允许按特定 POI 分类或交通站点类别筛选可选标记。道路端点组可绘制粗线近似轨迹，但必须用全量端点生成整条近似线，再判断整条线是否经过当前视口，不能只用可见范围内的端点决定是否展示；同时必须明确这不是道路级导航路线。
- 瓦片源切换：当同时配置实时 HTTP 代理瓦片和 HTTPS 精细静态瓦片时，地图页应允许在卫星模式下切换当前瓦片源，以便测试实时性与渲染质量之间的取舍；用户选择可保存在浏览器本地，若对应 Provider 不再可用则自动回退默认源。
...
- 地铁线路覆盖：已定位的地铁线路应作为地图底图覆盖层显示，三种浏览模式下都保留，线条颜色使用对应线路色；路线规划结果态不隐藏该地铁底图线，规划路线本身叠加在其上。线路没有单独途径坐标时，可先按已定位站点坐标和站序直线连接；如果旧交通站点表缺少 Minecraft 坐标，地图 API 可按站名匹配静态地图里的站点 POI 坐标生成临时折线，后续再用后台维护的精确坐标替换。
...
- 地点操作栏：普通点状地点详情需要提供操作栏，包括“查看路线”（重点按钮，图标使用 `directions`）、“搜索周边”、“收藏”和“分享”。道路、线路等线性地点不显示这组普通地点操作栏。当前工程第一版已支持查看路线、搜索周边、本机收藏和分享/复制地点链接；收藏会先写入浏览器本地 `yct.mapFavorites.v1`。登录用户打开账号页时会通过 `/api/account/map-favorites` 合并本机和账号侧 markerId 列表，写入 `.yct-data/map-favorite-store.json` 并发布 `MapFavoritesUpdated` 事件；当前只同步 markerId，不同步地点快照或跨设备删除冲突。
- 线路图层：地铁、有轨、公交、轮渡等线路可叠加展示。
- 线路详情：站点列表、方向、首末班、票价、运营方、相关公告。公共交通线路统一通过 `/map?marker=transit-line-...` 在地图上打开线路轨迹和小面板，线路 chip 不再跳离地图；`/travel/[id]` 已彻底弃用，`/map/lines/[id]` 暂时保留为地图侧线路详情兼容入口。
- 线路地图交互：车站详情中的接驳线路标识应可点击，点击后在地图页选中对应线路对象，详情面板展示线路信息，地图区域展示线路上所有已定位站点并高亮当前已知轨迹。当前没有后台线路坐标时，可用真实站点坐标形成近似展示轨迹，但不能宣称为道路级导航。地图页已支持通过 URL 参数聚焦对象：`marker=<markerId>`、`marker=transit-line-<lineId>`、`line=<lineId>`、`lineId=<lineId>` 都会尝试定位并打开对应地点或线路，且同一地图页内 URL 参数变化时会重新聚焦；后台审核等非公开对象可用 `x=<MinecraftX>&z=<MinecraftZ>&label=<名称>` 或 `coordinate=<x,z>` 打开临时坐标定位。
- 地图内线路详情：站点时间线节点和连接线需要使用线路色或对应交通方式语义色；站点列表需要提供正向/反向分段控制。加载特定方向时必须尊重停靠点属性中的单向站、停靠方向或后续后台方向数据，不能简单把站点数组反转后当作完整反向线路。旧线路数据中 `oneWay: down` 表示与数据记载方向相同，`oneWay: up` 表示与数据记载方向相反，迁移和前台方向过滤不能反用。
- 线路型 POI：公共交通线路后续需要作为一类或两类独立 POI 入库，至少记录线路方向、途径站点点标记和展示/检索属性；可以按需拆出方向子 POI。途径坐标由线路决定：数据质量足够或展示要求高的线路可手工填写方向轨迹，其他线路可以遵守道路级规划按途径站点生成可解释路线，不能凭站名直接画假线。
- 兴趣点管理：
  - 登录用户可创建私有兴趣点。
...
- 当用户关闭地图瓦片时，可以显示道路类对象的粗线条轨迹作为可读底图；但旧道路端点没有严格顺序，不能直接把同名端点按数组顺序当作正式路线。当前第一版采用端点近似排序生成视觉轨迹，并明确不代表道路级导航；后续仍需要后台道路网络、手工连接和自动抽取结合，形成可解释的导航节点与边。
- `map-data-badge`、`map-data-panel` 这类数据源、标记数量、适配器状态说明，对普通用户价值有限，不应占据地图页主要可见区域；必要时只保留“数据更新时间/来源”的轻量入口，详细诊断信息放入管理员、调试或关于面板。
- 用户上传公共 POI 前必须登录；公开前必须人工审核。POI 简介、外部链接和图片同样属于审核内容；第一阶段支持图片文件上传和图片链接兜底，文件上传限制常见图片格式与大小，并用文件签名做基础类型校验，生成可进入 POI 审核流的图片路径。后续仍需要补充对象存储、违规图片处理、完整图片安全扫描和更完整的管理员预览规则。
...
- 面性标记同时兼容多个矩形组合和多边形两种数据方式。多个矩形更贴近 Minecraft 方块世界和后台录入习惯，多边形适合已经整理好的区域边界；显示层转换为多边形或合并轮廓展示，编辑层保留原始矩形/多边形来源。
- 工程过渡实现：在数据库/Prisma 接入前，公开 POI 投稿使用 `.yct-data/poi-submission-store.json` 作为本地仓储，POI 投稿图片默认落盘到 `.yct-data/poi-submission-images`；前台 `/api/map/poi-submissions` 仅允许已登录且服务器账号已验证的 `ldpass` 用户提交公开 POI，后台 `/api/admin/map/poi-submissions` 由 YCT 管理员审核和发布。当前第一版投稿资料包括地点名称、分类、简介、相关链接、上传图片或图片链接和点坐标；上传图片写入投稿时使用不含临时部署前缀的内部路径，页面展示时再按当前部署前缀解析，避免 `/v2` 测试路径污染业务数据。
...
- 订单草稿第一版：统一班次查询页仅在服务端票务预检返回 `order_available` 时启用“创建草稿”按钮；按钮会调用 `POST /api/travel/ticketing/orders`，要求真实 `ldpass` Active 用户，并创建 15 分钟库存占用的 `draft` 订单。班次查询页和账号设置页都需要提供票务草稿面板，允许当前用户查看占座中的草稿并取消自己的 `draft` / `pending_issue` 订单以释放库存。账号页如果没有当前班次查询上下文，只能展示订单号、服务类型、人数和状态等订单自身信息，不得补造班次标题。该草稿不代表已支付、已出票或可核销票券；缺少票种、库存、登录态或可售性时必须返回明确错误并不落库。
- 占座过期处理：订单列表、详情、徽标计数和创建草稿等工作流入口读取票务订单前，需要先回写过期占座。过期的 `draft` 订单应变为 `expired`，库存占用应变为 `expired` 并释放容量，同时发出系统触发的 `TicketOrderCancelled` 事件，`reason` 为 `inventory_expired`。过期草稿不应继续计入账号徽标的待处理订单数。内部计划任务 runner 也应主动执行这项清理，避免长期无人访问时过期草稿滞留。
- 订单详情第一版：需要提供当前用户自己的订单详情读取 API 和二级页面，展示订单 ID、状态、服务类型、班次 ID、票种 ID、人数、库存占用、创建/更新时间与取消原因等服务端真实字段。详情页可以继续提供取消草稿操作，但不得展示支付成功、电子票、条形码、核销链接等尚未接入的凭证能力。
- 历史行程：登录后跨设备同步，本地匿名用户可保留浏览器本地历史。
- 线路浏览：完整线路列表、线路介绍和线路型 POI 已逐步转入地图探索；出行页第一版不再铺开完整线路列表，只保留“线路与站点”任务入口导向地图页。旧线路数据仍通过地图线路详情和全局搜索访问，不再提供 `/travel/[id]` 或 `/travel/stations/[lineName]/[stationName]` 二级页。
...
- 第一阶段出行页只保留“出行服务”和“行程提醒”两个主模块；统一班次查询入口、票务平台预留、客运大屏和线路/站点导向均收纳到“出行服务”中。当前出行服务已提供“班次查询”“线路与站点”“智运大屏”和“票券与订单”预留卡片，其中线路与站点入口直接导向地图探索。线路基础浏览、线路介绍和线路型 POI 转入地图探索。
...
- 当前工程状态：顶部头像通过 `/api/account/status` 读取账号状态；未配置/只读/会话不可用显示状态点，Active 登录显示登录态，YCT 管理员会合并显示内容、服务入口、交通数据和 POI 待审核数量；Active 用户存在 `draft`、`pending_issue` 或 `manual_review` 票务订单时，也会把待处理票务订单数合并进同一个账号徽标。前台还会读取浏览器本地行程提醒中的待同步数量，与服务端待办合并成一个徽标；票务订单创建或取消成功后会触发前端账号状态刷新；普通用户不会暴露管理员计数。头像优先使用 `ldpass` 返回的自定义头像，但若返回旧 `mc-heads.net` 地址则强制改用 `https://minotar.net/helm/<user>`；没有自定义头像时，YCT 也会按服务器账号名或用户名回退到 Minotar，头像框按 Minecraft 头像特征使用圆角矩形。
- 通知设置文案：账号页需要明确区分“通知类型开关”和“免打扰时段”。通知类型决定是否接收行程、运营、票务、检票等推送；免打扰时段决定指定时段内是否静默、延后或阻止通知，不能让用户误以为一个开关同时控制所有通知权限。当前账号页已把标题改为“推送通知与免打扰时段”，并明确说明总开关只控制本设备是否接收 Push，免打扰时段不会关闭通知分类，只会静默或延后提醒。
...
- 近期访问的 `/travel/schedules`、`/travel/screen` 与 `/operations/[id]` 会进入运行时缓存，用于弱网或离线时继续打开最近内容；`/travel/screen` 作为公共出行二级页也会随应用壳预热。旧线路页 `/travel/[id]` 和旧站点页 `/travel/stations/[lineName]/[stationName]` 已彻底弃用。
...
- 当前后台已提供“名称翻译”维护页，可按地图地点、线路和站点筛选，维护繁体中文与英文译名；地图、地图搜索和交通概览读取同一译名仓储，保存后发布 `EntityTranslationUpdated` 领域事件。
- 当前工程第一版已提供登录用户语言偏好 API `/api/account/locale-preference`，支持保存 `system`、`zh-CN`、`zh-Hant`、`en`，并发布 `LocalePreferenceUpdated` 事件；前端已有匿名本地偏好、登录同步工具和账号页语言偏好控件。全局 `PreferenceBridge` 会在页面进入时应用本地语言偏好、同步账号侧偏好、更新页面 `lang`，已接入 `useI18n` 的主导航、顶部常用入口、账号页外观/语言/动态偏好控件、搜索页、离线页、首页运营信息、更多服务页、出行一级页、行程提醒面板、统一班次查询页标题/筛选/本地记录/提醒公告/班次卡片基础操作/票务按钮、站点详情页层级/出入口/设施/换乘周边/数据来源固定文案、线路详情页概览/方向/站点列表/单向停靠/站点摘要/数据来源固定文案、票务草稿面板、票务订单详情页、订单状态/服务类型/取消原因/库存占用状态固定文案、地图页标题、地图搜索框、标记列表空态、附近搜索提示、POI 常用操作和路线规划卡片端点编辑/交通方式/候选方案/步骤详情/估算说明固定文案会随偏好切换。后续继续按模块扩展地图复杂面板、后台和更多业务固定文案。
...
- 内容审核第一阶段不做二审。
- 内容审核第一阶段暂时不需要自动敏感词、外链域名提示或图片安全自动处理等功能；人工审核先保证可用闭环。
- 线路数据发布需要预览。
- 预览环境通过后台嵌入预览和独立预览 URL 两种方式实现，便于管理员在后台快速检查，也便于把特定预览链接发给相关人员确认。
