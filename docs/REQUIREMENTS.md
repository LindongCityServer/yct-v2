...
- 当前工程补充：道路图和点状标记的道路投影索引会在地图数据阶段预先构建；每个标记点按道路只保留范围内的最近投影，所有投影点都必须在标记点 50 格范围内，不再允许第一个最近投影绕过范围限制。路线端点、出入口、公交/客运站点之间的道路段会优先使用这些固定投影；如果标记点没有范围内投影，则不把该标记点强行吸附到远处道路，避免斜穿街区。父地点与子地点互相规划时不强制道路投影，出入口类子地点不会自动改走同父地点的其它出入口。路线交通方式开关采用短延迟批量重算；路线方案摘要时间按已生成步行/道路接驳段估算，避免比详情时间明显偏短。
