...
- 订单草稿第一版：统一班次查询页仅在服务端票务预检返回 `order_available` 时启用“创建草稿”按钮；按钮会调用 `POST /api/travel/ticketing/orders`，要求真实 `ldpass` Active 用户，并创建 15 分钟库存占用的 `draft` 订单。该草稿不代表已支付、已出票或可核销票券；缺少票种、库存、登录态或可售性时必须返回明确错误并不落库。
