...
- 道路网络连通规则：从一条道路转向另一条道路时，后台道路图应能表达交叉、连接点和可通行规则；旧道路过渡图在中心线真实平面相交处推断路口，并允许在后台配置的端点吸附容差内推断受约束的丁字路口。吸附只针对真实道路端点，需排除近似平行和候选歧义，不能把任意邻近道路末端自动补成连接边。正式路线规划必须保留可解释的节点、边、权重和来源，不能把视觉粗线直接当作导航图。
- 公共交通路线规划：启用公共交通时，路线应由起点步行接驳、公共交通站点间线路段、换乘/步行段和终点接驳组合而成；公共交通线路段必须使用真实线路、站点和方向数据，不能凭站名顺序或近似轨迹伪造可乘坐路线。
- 当前工程状态：地图 POI 详情中的“查看路线”已能创建路线规划草稿，并生成第一版估算路线结果。步行方案保留直线估算，并新增基于旧道路中心线交点和受约束端点吸附的“道路步行估算”；该方案只作为过渡预览，不宣称为正式道路级导航。公共交通方案会按启用的交通方式遍历真实线路和方向，使用线路站序、站点标记和区间用时生成直达或一次换乘候选，排序后展示多条可能路径并在地图上高亮。路线卡片会标记“最快到达”“最少换乘”“最少步行”等候选特征，步骤时间线按地点、步行、乘车和换乘过程区分样式；路线结果态隐藏非路线相关标记，仅保留起终点和经过站点。区域/父子 POI、地铁出入口和普通出入口按各自规则接驳；公交站作为点状站点不再作为父 POI 吸收命名子地点。道路接入候选同时保留方向评分较优和距离最近的投影点。路线规划卡片支持分享并恢复起终点、交通方式和候选。地点远离道路时，直线只允许作为地点到道路的步行接驳；若道路图不连通才返回明确的直线估算，不得把它标成沿路导航。正式道路图、后台可编辑出入口/方向规则和更完整换乘权重仍待后续接入。
- 当前工程补充：道路图和点状标记的道路投影索引会在地图数据阶段预先构建；每个标记点按道路保留最近投影，其中距离最近的投影默认保留，其它投影仍需在标记点 50 格范围内。多个标记点可以共享同一个道路投影，投影只是本次路线的端点接入，不写回道路图，也不能被其它路线当作公共道路中继。公交站牌到道路的部分属于乘客步行接驳，不属于公交车辆走行；公交车辆段只能由道路边组成，任一相邻站段道路不连通时整条候选不可用，不得用两站直线或道路末端 V 形接驳冒充公交路线。父地点与子地点互相规划时不强制道路投影，出入口类子地点不会自动改走同父地点的其它出入口。路线交通方式开关采用短延迟批量重算；路线方案摘要时间按已生成步行/道路接驳段估算，避免比详情时间明显偏短。
- 当前工程补充：地图数据和瓦片可见后，如果道路图与标记点道路接入点仍在计算，地图区域需要显示状态提示；路线规划应等待接入索引 ready 后再生成候选，避免在索引未完成时先落入过度直线化的临时结果。
- 当前工程补充（2026-08-01）：旧道路运行时构图已取消“道路尽头距另一道路 100 格以内就自动连接”的过渡规则；现在只在旧中心线真实平面相交处，或后台配置的 30 格默认、最大 64 格的受约束端点吸附条件下推断路口。吸附会排除近似平行道路，并在候选接近时拒绝自动决定；尚未连通的路段不再人为补直线边。旧道路默认允许步行、出租车和公交共用；公交和出租车必须走道路，出租车首尾可附加步行接驳。路线候选先合并站序、方式、时间、步行量和票价等价的公共交通方案，再按同方式时间/步行/换乘支配关系淘汰明显劣势项，并为每个已启用且确有可行方案的交通方式保留至少一项。路线方式包含打车，标识色为明黄色；出租车耗时按道路里程、后台默认限速和路口延误估算，票价按后台可维护规则计算。
- 当前工程补充（2026-08-01）：路线排序支持时间、步行距离、换乘次数和票价。公交与出租车的默认速度和每路口延误均由地图后台维护；线路相邻站段可维护区间用时，规划优先级为“后台区间用时 > 旧数据 `travelTime` > 里程与路口延误估算”。公交、轮渡默认票价、出租车计价和轨道里程阶梯均可后台维护；连续地铁/有轨电车的计费里程取计费网络中两站间最短里程，实际方案绕路不加价。
- 当前工程补充（2026-08-01）：地图滚轮和加减按钮以地图安全框中心为缩放中心。路线首次生成或切换方案时自动取景；之后安全框变化仅在用户尚未手动缩放时再次取景，并且只允许保持倍率或缩小，展开路线详情不得放大规划路线。
- 当前工程补充（2026-08-01）：POI 后台几何地图编辑器已支持读取当前登录账号绑定且验证过的服务器玩家位置。Point 使用该位置替换当前坐标；MultiPoint、LineString、矩形和多边形使用该位置追加或扩展当前几何。现有玩家位置源只提供 X/Z，Y 暂按地图空间设置的默认高度解释，未绑定账号或没有位置记录时必须显示明确错误且不修改草稿。
- 当前工程补充（2026-08-01）：交通线路相邻站间路径已增加 `operating`、`planned`、`closed` 区段状态，旧数据缺省为 `operating`。可视化线路编辑器可以逐段维护状态；`planned` 只在对应交通方式启用“显示规划区段”时进入公开地图，并固定渲染为灰色虚线；`planned` 和 `closed` 均不进入普通路线规划。
...
- 编辑器瓦片兜底：地图选点控件、POI 审核/几何编辑器和其他后台地图编辑控件允许先加载 HTTPS 静态瓦片作为底图；当较新的 HTTP 瓦片可以通过 YCT 同源代理成功取得时，逐瓦片覆盖静态底图。较新瓦片请求失败、返回透明占位或内容类型无效时必须继续显示静态瓦片，不能把编辑器地图变成空白；静态源缺失时才允许单独使用较新源。
...
- 方向级停靠位置：同一线路站点可分别关联正向、反向和本线路默认位置。读取时优先使用方向专属位置，不存在时回退到本线路默认位置，再回退到普通站点标记；写入时必须校验站点属于当前线路，且位置标记的交通方式与线路一致。
...
- 内置物料工作台：路牌与公共交通导视使用统一工作台骨架，支持受限结构化编辑、实时预览和权限受控的导出；编辑建议必须来自真实线路、站点和地点数据。
- RMP 自定义线网：公共交通导视可导入[地铁线路图绘制器（Rail Map Painter，RMP）](https://railmapgen.org/?app=rmp)项目文件。导入后作为只读线网快照使用，不支持修改文件固有的站点、站序、站名或拓扑；这些信息必须在 RMP 中修改后重新导入。
- 常见问题：FAQ 固定内容至少覆盖账号、地图与线路、出行与提醒、物料工具、离线与通知、数据与隐私，并同步维护简体中文、繁體中文和 English。
...
- RMP 线网项目接口：登录用户可以导入、更新和删除自己的暂存项目，接口必须执行归属校验、文件版本与容量限制、Schema 校验，并通过领域事件通知后续审计或同步监听器。
...
- RMP 项目当前使用本地 JSON 保存每个登录用户的一份草稿，只适合单机验证；多实例环境需要共享持久化、乐观锁、配额限制和过期清理，避免不同实例相互覆盖或磁盘无限增长。
...
- 第三方项目格式漂移：RMP 文件版本、节点类型或线路属性升级后可能超出当前解析能力。导入必须拒绝未知主版本并给出兼容性说明，不能静默丢弃站点或拓扑后继续导出。
- 用户草稿并发覆盖：同一账号在两个设备上连续导入时，本地 JSON 的“最后写入者胜出”会无提示覆盖旧草稿；迁移共享存储时必须带版本号或更新时间条件更新。
...
- 方向停靠位置：正向与反向分别命中、缺少方向专属位置时回退、跨线路站点引用拒绝、交通方式不匹配拒绝。
- RMP 导入：合法版本导入、超大文件、节点或边超限、未知版本、缺少具名站点、无有效线路边、重新导入覆盖草稿、越权读取或删除拒绝。
...
- 地图空间、正规道路图、高度/楼层、交通运营状态、站内 2.5D、通道、打车路线、行政区划、动态编号图标和路线候选治理的专项边界统一记录在 `docs/MAP_SPATIAL_ROUTING_ARCHITECTURE.md`。后续实现必须先完成其中的道路小区域 MVP，不能继续把固定距离自动连接产生的近似道路图当作正式导航数据。
- 同一交通线路允许按区段标记运营状态。已运营区段使用线路色实线；规划区段统一使用灰色虚线，其普通地图可见性跟随对应交通方式配置。规划区段即使可见也不得进入普通路线规划。
- 当前主世界使用内部稳定 ID `lindong-overworld`，默认 Y 为 `64`，道路默认限速为 `60 km/h`，并在后台提供地图空间设置入口。外部 uNmINeD 页面只提供可变 `worldName`，不得直接作为持久化主键。
- 旧道路默认按中心线导入，平面交点推断为待复核路口；默认不存在桥隧和匝道，管理员可以在后台更正推断拓扑。邻近但不相交的道路不得再通过固定距离自动补连接边。
- 第一阶段不做可旋转真 3D。地铁站使用平面站体覆盖；其他立体 POI 的 2.5D 挤出只在非卫星模式提供，并允许用户关闭。
- 动态编号图标需要尽量完全复刻旧 PNG，采用旧图视觉基准与模板化 SVG/字体组合生成。
- 当前动态编号视觉规范：地铁出入口为白底灰边灰字；省道为琥珀底黑边黑字；高速公路保留白边并在文字与上边框之间增加 `#fff900` 横条，`G` 开头的高速编号使用红色横条。道路/高速公路存在 `S2`、`G25` 等短编号时，在道路标签锚点显示编号标识。
...
- 地图空间与路由专项架构：`docs/MAP_SPATIAL_ROUTING_ARCHITECTURE.md`
...
## 16. 地图与交通专项收口清单（2026-08-01）

本轮已将前述讨论收敛为可实施边界：

- 交通线路、站点和线路段的运营状态与审核发布状态分离。`planned` 可进入物料工作台，普通地图和路线规划只读取已公开且 `operating` 的实体；规划段统一灰色虚线，是否显示跟随交通方式配置。
- 地图空间设置默认 `worldId=lindong-overworld`、默认 Y=64、道路默认限速 60 km/h，均从后台配置读取；缺失 Y 继承默认值，不同 Y 不自动相交，只有显式匝道/楼梯/扶梯/电梯连接器跨层。
- 旧道路先按有序中心线迁移，默认无桥隧匝道、双向、允许人/出租车/公交/客车；平面交点和 30 格默认端点吸附只生成过渡图建议，管理员后续可修正拓扑。公交和出租车车辆段只能使用允许的道路边，步行通道默认只允许步行。
- 点、线、面编辑器支持使用或追加当前绑定玩家位置；写入的是带观测时间的位置快照，离线或失效位置不静默写入。
- POI 支持高度、半透明样式、线路色绑定、挤出矩形/多矩形/多边形体积和动态编号；2.5D 只在非卫星图层显示且可关闭，地铁站不挤出。动态符号显式配置优先，无法确认旧 PNG 视觉规则时保留 PNG 回退；像素级复刻需要旧 PNG 与数字/字母 SVG。
- 区域几何标记默认不显示边框，选中/聚焦时才恢复高亮描边，避免区域数量较多时地图被轮廓线淹没。
- `pedestrian-path`、`ordinary-entrance` 与 `exit-*` 已分离；行政区划使用独立聚合、父级链和地图层，不参与普通区域 POI 详情、搜索或路由。
- 路线规划提供步行、公交、轨道和明黄色打车；候选先按硬过滤、等价合并、支配淘汰，再按时间、步行、换乘次数或票价排序。出租车费用、速度、路口延误、公交区间用时和轨道计费阶梯全部后台可维护。

### 状态机与事件边界

状态机分为两条正交链：数据审核发布链（`draft -> pending_review -> approved -> published`）和运营链（`planned -> operating -> closed`，旧数据缺省为 `operating`）。Workflow 只写本领域仓储并发布领域事件；地图公开读取、物料目录、路线图、搜索索引和缓存由监听器更新，避免服务之间直接调用。

### 高频风险

- 把二维相交当作可通行路口会造成穿桥、穿楼和跨层误连，必须比较有效 Y 并要求显式连接器。
- 把地点到道路的接驳段写回道路图会让其它路线把 POI 当成道路中继，接驳边必须是本次路线的临时边。
- 单向边只限制主图而不限制“投影点到端点”的接入，会产生逆行；接入候选和同段路径必须同时检查方向。
- 只在浏览器内构图会在全量旧道路上触发 O(n²) 交点检测；正式道路图需离线分区、稳定节点 ID、版本化发布和可重放事件。
- 多实例部署若只依赖内存事件总线会丢更新；生产必须使用事务 Outbox、幂等监听器和失败重放。

### 最小验收用例

1. 同一平面相交道路可连通；不同 Y 无连接器时不可连通，有匝道时仅允许配置的交通方式跨层。
2. 单向道路反向不可达；公交无法进入 `pedestrian-path`；出租车不会使用步行通道或直线车辆段。
3. 远离道路的 POI 只生成步行接驳；道路图断开时返回不可达/估算并标明原因，不伪造沿路路线。
4. `planned` 线路可以生成物料但不会出现在普通路线规划；关闭站点/线路不会被公众搜索命中。
5. 仅线路名称或编号不同的同质候选合并，并保留所有线路 ID；启用的每种交通方式有可行结果时至少保留一个。
6. 行政区划父级链不能成环；公开行政区层不会出现在 POI 搜索或路线节点中；2.5D 在卫星模式强制关闭。
