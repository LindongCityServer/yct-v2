...
- 区域 POI 关联：区域/面性 POI 需要能关联多个点状 POI，并按出入口、设施、楼栋、景点等分组展示在详情的“设施或出入口”区域。区域 POI 可以指定代表 POI，默认代表可按同名点标记推导；地图标记碰撞检测应优先保留代表 POI 的图标和标签。当前工程第一版先在前端按旧地图命名规则推导只读关联：若存在父地点 `XXXX`，则 `XXXX-OOOO` 会在父地点详情的“设施/出入口”中显示为子地点入口，并支持点按聚焦子 POI；正式分组、代表点和后台编辑仍待后续实现。
