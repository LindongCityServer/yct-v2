...
- 当前工程状态：内容后台已具备本地内容素材审核闭环，可上传新素材到 `apps/web/public/content-assets`，也可把旧内容素材清单导入 `.yct-data/content-asset-store.json`，记录来源、哈希、引用关系和审核状态；同 SHA-256 素材会复用已有记录，素材审核通过后，带 `assetIds` 的内容才允许发布。
...
- 图片上传与插入：正文图片必须先作为内容素材上传并进入审核；上传后可把素材 ID 绑定到内容草稿，并在 Markdown 中引用同站公开路径。外链图片需要记录来源、展示域名和审核状态。
