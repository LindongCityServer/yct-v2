...
- 当前工程状态：顶部头像通过 `/api/account/status` 读取账号状态；未配置/只读/会话不可用显示状态点，Active 登录显示登录态，YCT 管理员会合并显示内容、服务入口、交通数据和 POI 待审核数量；Active 用户存在 `draft`、`pending_issue` 或 `manual_review` 票务订单时，也会把待处理票务订单数合并进同一个账号徽标。前台还会读取浏览器本地行程提醒中的待同步数量，与服务端待办合并成一个徽标；普通用户不会暴露管理员计数。
