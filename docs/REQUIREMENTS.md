...
- 图层与投稿：`map-layer-panel` 第一阶段保留浏览模式、标记点/线条显示开关和投稿 POI 的弹窗入口；完整图层列表和常驻投稿表单后续再展开。公开 POI 投稿表单需要支持“在地图上选点”回填坐标；后台中涉及坐标、站点、线路途径点和几何编辑的字段也应复用地图选点能力，并允许按特定 POI 分类或交通站点类别筛选可选标记。道路端点组可绘制粗线近似轨迹，但必须用全量端点生成整条近似线，再判断整条线是否经过当前视口，不能只用可见范围内的端点决定是否展示；同时必须明确这不是道路级导航路线。
...
- 当前工程状态：顶部头像通过 `/api/account/status` 读取账号状态；未配置/只读/会话不可用显示状态点，Active 登录显示登录态，YCT 管理员会合并显示内容、服务入口、交通数据和 POI 待审核数量；Active 用户存在 `draft`、`pending_issue` 或 `manual_review` 票务订单时，也会把待处理票务订单数合并进同一个账号徽标。前台还会读取浏览器本地行程提醒中的待同步数量，与服务端待办合并成一个徽标；票务订单创建或取消成功后会触发前端账号状态刷新；普通用户不会暴露管理员计数。
