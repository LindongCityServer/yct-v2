...
- 占座过期处理：订单列表、详情、徽标计数和创建草稿等工作流入口读取票务订单前，需要先回写过期占座。过期的 `draft` 订单应变为 `expired`，库存占用应变为 `expired` 并释放容量，同时发出系统触发的 `TicketOrderCancelled` 事件，`reason` 为 `inventory_expired`。过期草稿不应继续计入账号徽标的待处理订单数。
