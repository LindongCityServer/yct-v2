...
- 当前工程状态：地图 POI 详情中的“查看路线”已能创建路线规划草稿，记录地图中心起点、目的地点名和目的地坐标，供后续道路图算法接入。该草稿不生成路线、不估算耗时，也不把旧道路端点粗线当作正式导航结果。
...
- 搜索结果页需要提供分类 chip 栏，用于切换或筛选运营信息、地点/POI、线路、服务入口、工具等结果类型，避免某一类结果过多时压住其他类型结果。
