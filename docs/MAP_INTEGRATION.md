...
- 线路停靠位置：
  - 后台可为线路中的每个站点分别关联“本线路默认位置”“正向”和“反向”地图标记。
  - 保存时会校验站点属于当前线路、标记分类的交通方式与线路一致，并规范化、去重引用。
  - 地图读取时优先使用当前方向的专属位置；不存在时依次回退到本线路默认位置和普通站点标记。
...
- 已处理：公共交通线路详情统一通过 `/map?marker=transit-line-...` 打开地图线路轨迹和小面板；地图线路型 POI、全局搜索线路结果和出行页线路列表使用同一入口。旧 `/map/lines/[id]` 与 `/travel/[id]` 仅保留兼容。出行页后续仍需继续精简，只保留提醒、客运班次查询、订票与后续交通方式预留入口。
...
- 已处理第一版：线路停靠点支持按正向、反向或本线路默认位置关联地图标记；方向专属位置不存在时会稳定回退，避免一条线路在两个方向共用站名但实际停靠位置不同的场景中定位错误。该方向是线路运营方向，不与 RMP 导视中按图几何推导的东、西、南、北混用。
...
- 已处理第一版：地图路线规划卡片改为起终点胶囊行、交通方式开关、方案摘要和步骤时间线组合；支持交通方式全选/全关、轮渡开关、起终点交换，并继续在地图区域高亮当前选中方案的估算轨迹。卡片已限制最大高度，方案列表内部滚动，移动端控制栈贴在底部导航上方。起点/终点行可进入编辑状态，支持输入搜索真实可定位标记候选、点选候选或地图标记回填端点，也支持轻点地图空白处回填 Minecraft 坐标。当前仍是基于可用站点和直线距离的估算，不代表道路级正式导航；公共交通候选会过滤起终点同站进出的无效结果。
- 已处理第一版：旧地图标记名称中形如 `XXXX-OOOO` 的条目会保留完整搜索名称，同时派生 `parentLabel=XXXX` 和 `secondaryLabel=OOOO`，供后续区域 POI 绑定、出入口/设施分组和代表 POI 优先级使用。
