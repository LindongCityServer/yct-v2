...
- 已处理第一版：地图路线规划卡片改为起终点胶囊行、交通方式开关、方案摘要和步骤时间线组合；支持交通方式全选/全关、轮渡开关、起终点交换，并继续在地图区域高亮当前选中方案的估算轨迹。卡片已限制最大高度，方案列表内部滚动，移动端控制栈贴在底部导航上方。当前仍是基于可用站点和直线距离的估算，不代表道路级正式导航；公共交通候选会过滤起终点同站进出的无效结果。
